...
LPCTSTR __stdcall GetFaultReason(EXCEPTION_POINTERS* pExPtrs)
{
    ...
    __try
    {
        ...
        {
            iCurr +=
                BSUGetModuleBaseName(GetCurrentProcess(), (HINSTANCE)dwTemp,
                        ...
                        &dwDisp, &g_stLine))
        {
            ...
            else
            {
                ...
                {
                    iCurr += wsprintf(
                        g_szBuff + iCurr, _T("%s, line %d+%d byte(s)"),
                                    ...
                                    LPDWORD lpNumberOfBytesRead)
{
    ...
    SIZE_T got = 0;
    BOOL r = ReadProcessMemory(GetCurrentProcess(), lpBaseAddress, lpBuffer,
                               nSize, &got);
...
#define CH_MACHINE IMAGE_FILE_MACHINE_I386
        ...
        BOOL bSWRet =
            StackWalk(CH_MACHINE, (HANDLE)GetCurrentProcessId(),
...
#ifdef _ALPHA_
        iCurr +=
            wsprintf(g_szBuff + iCurr, _T("0x%08X"), g_stFrame.AddrPC.Offset);
#else
        iCurr +=
            wsprintf(g_szBuff + iCurr, _T("%04X:%08X"),
        ...
        if (GSTSO_SYMBOL == (dwOpts bitand GSTSO_SYMBOL))
        {
...
#endif
            {
                ...
                {
                    lstrcpyn(g_szBuff + iCurr, pSym->Name,
                             BUFF_SIZE - iCurr - 1);
                    // Gotta leave now.
                    szRet = g_szBuff;
                    __leave;
                ...
                else
                {
                    ...
                    {
                        iCurr +=
                            wsprintf(g_szBuff + iCurr, _T("%s()+%d byte(s)"),
                                     pSym->Name, dwDisp);
                    }
                    else
                    {
        ...
        if (GSTSO_SRCLINE == (dwOpts bitand GSTSO_SRCLINE))
        {
                            ...
                            g_stFrame.AddrPC.Offset, &dwDisp, &g_stLine))
            {
                ...
                else
                {
                    ...
                    {
                        iCurr +=
                            wsprintf(g_szBuff + iCurr, _T("%s, line %d"),
                                     g_stLine.FileName, g_stLine.LineNumber);
                    }
                }
            }
                                ...
                                OUT PIMAGEHLP_LINE Line)
{
    ...
    if (NULL not_eq g_pfnSymGetLineFromAddr)
    {
        ...
        return (
            g_pfnSymGetLineFromAddr(hProcess, dwAddr, pdwDisplacement, Line));
#endif
    }
...
void InitSymEng(void)
{
    if (FALSE == g_bSymEngInit)
    {
        ...
        VERIFY(SymInitialize((HANDLE)GetCurrentProcessId(), NULL, FALSE));
        UINT uiCount;
        // Find out how many modules there are.
        VERIFY(GetLoadedModules(GetCurrentProcessId(), 0, NULL, &uiCount));
        // Allocate something big enough to hold the list.
        HMODULE* paMods = new HMODULE[uiCount];

        // Get the list for real.
        if (FALSE ==
            GetLoadedModules(GetCurrentProcessId(), uiCount, paMods, &uiCount))
        {
            ASSERT(FALSE);
        ...
        for (UINT uiCurr = 0; uiCurr < uiCount; uiCurr++)
        {
            ...
            VERIFY(GetModuleFileName(paMods[uiCurr], szModName,
                                     sizeof(szModName)));
            ...
            //  ends.
            HANDLE hFile = CreateFile(szModName, GENERIC_READ, FILE_SHARE_READ,
                                      NULL, OPEN_EXISTING, 0, 0);

            VERIFY(SymLoadModule((HANDLE)GetCurrentProcessId(), hFile,
                                 szModName, NULL, (DWORD)paMods[uiCurr], 0));
        }
...
BOOL BUGSUTIL_DLLINTERFACE __stdcall IsMiniDumpFunctionAvailable(void)
{
    ...
    if (NULL == g_pfnMDWD)
    {
        ...
        if (NULL not_eq hInstDBGHELP)
        {
            ...
            // function.
            g_pfnMDWD = (PFNMINIDUMPWRITEDUMP)GetProcAddress(
                hInstDBGHELP, k_MINIDUMPWRITEDUMP);
                              ...
                              DWORD dwThread, EXCEPTION_POINTERS* pExceptInfo)
{
    ...
    wchar_t* pWideFileName = (wchar_t*)HeapAlloc(
        GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS | HEAP_ZERO_MEMORY, iLen);

    int iRet = MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, szFileName, -1,
                                   pWideFileName, iLen);
    ...
    if (iRet not_eq 0)
    {
        ...
        eRetVal = CreateCurrentProcessMiniDumpW(eType, pWideFileName, dwThread,
                                                pExceptInfo);
    }
    else
    {
                              ...
                              DWORD dwThread, EXCEPTION_POINTERS* pExceptInfo)
{
    ...
    {
        ASSERT(FALSE == IsBadReadPtr(pExceptInfo, sizeof(EXCEPTION_POINTERS)));

        if (TRUE == IsBadReadPtr(pExceptInfo, sizeof(EXCEPTION_POINTERS)))
        {
            return (eBAD_PARAM);
        }
    }
    ...
    unsigned dwTID;
    HANDLE hThread =
        (HANDLE)_beginthreadex(NULL, 0, DumpThread, &stParams, 0, &dwTID);
...
unsigned WINAPI DumpThread(LPVOID pData)
{
    ...
    HANDLE hFile =
        CreateFileW(pParams->szFileName, GENERIC_READ bitor GENERIC_WRITE, 0,
    ...
    if (INVALID_HANDLE_VALUE not_eq hFile)
    {
        ...
        BOOL bRet = g_pfnMDWD(GetCurrentProcess(), GetCurrentProcessId(), hFile,
                              pParams->eType, pMDEI, NULL, NULL);
        ASSERT(TRUE == bRet);
