...
bool g_bCampaignAddMission =
    false; // Artscout - 2026: offer "Add Flight" and "Add Package" when right-clicking a target in the CAMPAIGN, the way BMS does. None of the machinery is new -- tactical_add_flight / tactical_add_package take the right-clicked object's VU_ID, GetMissionFromTarget picks the mission type that suits that target, and tactical_make_package files the MissionRequest. SetupCampaignMenus simply hid the items outside the Tactical Engagement editor, and this un-hides them. The request is tagged REQF_TE_MISSION, so how well a hand-built package plays with a running ATO is the part that is NOT settled -- set 0 to put the items back in hiding. TESTED AND NOT YET WORKING, hence the default: the items appear but do nothing, because tactical_add_flight / tactical_add_package look up TAC_FLIGHT_WIN and PACKAGE_WIN, and those live in te_scf.lst which only the Tactical Engagement screen loads -- the campaign screen loads cp_scf.lst. Every guard is on FindWindow being non-NULL, so it no-ops rather than crashing. Making it real means loading the TE window set alongside the campaign one and hooking up its controls, which is a bigger change than un-hiding a menu item. "CampaignAddMission".
