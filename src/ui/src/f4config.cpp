...
bool g_bCampFlotLine =
    true; // Artscout - 2026: draw the forward line of own troops on the campaign map, the way Allied Force does. The data has always been there and live -- RebuildFLOTList (camplib/camplist.cpp) takes the midpoint of every link between two frontline objectives on opposing teams, thins them to 30 km apart, and gamemgr.cpp rebuilds the list as the war moves -- but it had only ever been read for distance-to-front arithmetic and nothing drew it. Map right-click -> FLOT line toggles it; this is the master switch. Note the list is sorted along one axis (FLOTSortDirection picks x or y) and RebuildFLOTList warns in its own comment that this looks wrong where a front doubles back on itself, so expect a zigzag on a theater whose line is not broadly straight. Korea runs east-west and traces correctly. 0 removes the menu item as well. "CampFlotLine".
