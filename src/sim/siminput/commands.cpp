...
extern bool
    g_bExtViewOnGround; // RAS -5Dec04- Allow external sat/orbit view while on ground
...
void ToggleVtune(unsigned long, int state, void*)
{
    if (not hlib)
    {
        ...
        resumeFn =
            (int(__cdecl*)(void))GetProcAddress(hlib, "VtResumeSampling");
...
void ToggleSmoke(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        FalconTrackMessage* trackMsg = new FalconTrackMessage(
            1, SimDriver.GetPlayerAircraft()->Id(), FalconLocalGame);
        ShiAssert(trackMsg);
...
void SimRadarAAModeStep(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        RadarClass* theRadar = (RadarClass*)FindSensor(
            SimDriver.GetPlayerAircraft(), SensorClass::Radar);
...
void SimRadarNextTarget(unsigned long, int state, void*)
{
    ...
    if (SimDriver.GetPlayerAircraft() and
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP) and
        (state bitand KEY_DOWN) and not g_bRealisticAvionics)
    {
        RadarClass* theRadar = (RadarClass*)FindSensor(
            SimDriver.GetPlayerAircraft(), SensorClass::Radar);
...
void SimMaverickFOVStep(unsigned long, int state, void*)
{
    ...
    if (pac and pac->IsSetFlag(MOTION_OWNSHIP) and (state bitand KEY_DOWN) and
        pac->Sms)
    {
        if (pac->Sms->curWeaponType == wtAgm65 and pac->Sms->curWeapon.get())
        {
            MaverickDisplayClass* mavDisplay =
                (MaverickDisplayClass*)((MissileClass*)
                                            pac->Sms->curWeapon.get())
                    ->display;
            ...
            if (OTWDriver.GetVirtualCockpit())
                if (theRwr->IsPriority() not_eq FALSE)
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_PRIORITY, 2);
                else
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_PRIORITY, 1);
        }
    }
}
        ...
        if (theRwr)
        {
            ...
            if (OTWDriver.GetVirtualCockpit())
                if (theRwr->TargetSep() not_eq FALSE)
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_TGT_SEP, 2);
                else
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_TGT_SEP, 1);
        }
    }
}
        ...
        if (theRwr)
        {
            ...
            if (OTWDriver.GetVirtualCockpit())
                if (theRwr->ShowUnknowns() not_eq FALSE)
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_UNKS, 2);
                else
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_UNKS, 1);
        }
    }
}
        ...
        if (theRwr)
        {
            ...
            if (OTWDriver.GetVirtualCockpit())
                if (theRwr->ShowNaval() not_eq FALSE)
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_NAVAL, 2);
                else
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_NAVAL, 1);
        }
    }
}
        ...
        if (theRwr)
        {
            ...
            if (OTWDriver.GetVirtualCockpit())
                if (theRwr->ShowLowAltPriority() not_eq FALSE)
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_GND_PRI, 2);
                else
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_GND_PRI, 1);
        }
    }
}
        ...
        if (theRwr)
        {
            ...
            if (OTWDriver.GetVirtualCockpit())
                if (theRwr->ShowSearch() not_eq FALSE)
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_SEARCH, 2);
                else
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_SEARCH, 1);
        }
    }
}
...
void SimPickle(unsigned long, int state, void*)
{
    if (state bitand KEY_DOWN)
    {
        ...
        if (g_bRealisticAvionics)
        {
            if (SimDriver.GetPlayerAircraft() and
                SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP) and
                SimDriver.GetPlayerAircraft()->AVTRState(
                    AircraftClass::AVTRStateFlags::
                        AVTR_AUTO)) // 2002-02-15 MODIFIED BY S.G. Check if MOTION_OWNSHIP before going in otherwise it might CTD just after eject
            {
                if (SimDriver.GetPlayerAircraft()->Sms and
                    SimDriver.GetPlayerAircraft()->Sms->MasterArm() ==
                        SMSBaseClass::Safe)
                    return;
            ...
            if (SimDriver
                    .GetPlayerAircraft() and // MLR 5/4/2004 - <-- THIS IS NULL Fixing pickle-after-ejaculate-CTD.
            ...
            // and not ((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectCountdown) //Wombat778 4-02-04 Removed, as the ejectcountdown value is not a stable number, and causes the laser NEVER to fire automatically
            {
                if (SimDriver.GetPlayerAircraft()->FCC and
                    SimDriver.GetPlayerAircraft()->FCC->LaserArm and
                    SimDriver.GetPlayerAircraft()->FCC->GetMasterMode() ==
                        FireControlComputer::AirGroundLaser)
                {
                    SimDriver.GetPlayerAircraft()->FCC->CheckForLaserFire =
                        TRUE;
...
void SimTrigger(unsigned long, int state, void*)
{
    if (state bitand KEY_DOWN)
    {
        ...
        if (SimDriver.GetPlayerAircraft() and
            ...
            SimDriver.GetPlayerAircraft()->AVTRState(
                AircraftClass::AVTRStateFlags::
                    AVTR_AUTO)) // 2002-02-15 ADDED BY S.G. Check if MOTION_OWNSHIP before going in otherwise it might CTD just after eject
        {
            if (SimDriver.GetPlayerAircraft()->Sms and
                SimDriver.GetPlayerAircraft()->Sms->MasterArm() ==
                    SMSBaseClass::Safe)
...
void SimMissileStep(unsigned long, int state,
                    void*) //me123 addet nosewheel stearing
{
        ...
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP))
    {
        if (not SimDriver.GetPlayerAircraft()->af->IsSet(AirframeClass::InAir))
        {
            ...
            if (g_bRealisticAvionics)
            {
                if (state bitand KEY_DOWN)
                {
                    if (not SimDriver.GetPlayerAircraft()->af->IsSet(
                            AirframeClass::NoseSteerOn))
                        SimDriver.GetPlayerAircraft()->af->SetFlag(
                            AirframeClass::NoseSteerOn);
                    else
        ...
        else
        {
            ...
            if (SimLibElapsedTime - mslsteptimer >= 500)
            {
                ...
                if (masterMode == FireControlComputer::Missile or
                    masterMode == FireControlComputer::Dogfight or
                    masterMode == FireControlComputer::MissileOverride or
                    masterMode ==
                        FireControlComputer::AAGun /*Cobra TJL 11/12/04*/
                )
                {
                    SimDriver.GetPlayerAircraft()->Sms->StepAAWeapon();
                }

                if (masterMode == FireControlComputer::AirGroundBomb or
                    masterMode == FireControlComputer::AirGroundMissile or
                    masterMode == FireControlComputer::AirGroundHARM or
                    masterMode == FireControlComputer::AirGroundLaser or
...
void SimCursorFunc(int state, int xOff, int yOff)
{
    ...
    if (pac and pac->IsSetFlag(MOTION_OWNSHIP))
    {
        RadarDopplerClass* theRadar =
            (RadarDopplerClass*)FindSensor(pac, SensorClass::Radar);
        FireControlComputer* pFCC = pac->GetFCC();
        ...
        if (state bitand KEY_DOWN)
        {
            ...
            HarmTargetingPod* theHTS =
                (HarmTargetingPod*)FindSensor(pac, SensorClass::HTS);

            if (pac->Sms->curWeaponType == wtAgm65 and
                pac->Sms->GetCurrentWeapon())
            {
                mavDisplay =
                    (MaverickDisplayClass*)((MissileClass*)
                                                pac->Sms->curWeapon.get())
        ...
        (state bitand KEY_DOWN))
    {
        ...
        case APEnhanced:
            if (not SimDriver.GetPlayerAircraft()->OnGround() or
                SimDriver.GetPlayerAircraft()->AutopilotType() ==
...
void SimSelectSRMOverride(unsigned long, int state, void*)
{
    if (SimDriver.GetPlayerAircraft() and
        ...
        (state bitand KEY_DOWN))
    {
        if (SimDriver.GetPlayerAircraft()->FCC->GetMasterMode() ==
        ...
        SimDriver.GetPlayerAircraft()
            ->FCC->EnterDogfightMode(); // MLR 4/11/2004 -
        MfdDisplay[1]->SetNewMasterMode(
            SimDriver.GetPlayerAircraft()->FCC->GetMainMasterMode());

        if (SimDriver.GetPlayerAircraft()->Sms and
            SimDriver.GetPlayerAircraft()->Sms->drawable)
            SimDriver.GetPlayerAircraft()->Sms->drawable->SetDisplayMode(
                SmsDrawable::Wpn);

        //MI 02/02/02
        if (SimDriver.GetPlayerAircraft()->Sms and
            SimDriver.GetPlayerAircraft()->Sms->GetCoolState() ==
                SMSClass::WARM and
...
void SimSelectMRMOverride(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        SimDriver.GetPlayerAircraft()
            ->FCC->EnterMissileOverrideMode(); // MLR 4/11/2004 -
        MfdDisplay[1]->SetNewMasterMode(
            SimDriver.GetPlayerAircraft()->FCC->GetMainMasterMode());
...
void SimToggleMissileCage(unsigned long, int state, void*)
{
    ...
    if (pac and pac->IsSetFlag(MOTION_OWNSHIP) and (state bitand KEY_DOWN))
    {
        ...
        if (pac->Sms->curWeaponType == wtAgm65 and pac->Sms->curWeapon.get() and
            pac->Sms->Powered)
        {
            static_cast<MissileClass*>(pac->Sms->curWeapon.get())->Covered =
                FALSE;
            return;
        }
...
void SimToggleMissileTDBPUncage(unsigned long val, int state, void*)
{
    ...
    {
        SimWeaponClass* wpn =
            SimDriver.GetPlayerAircraft()->Sms->GetCurrentWeapon();
...
void SimDropChaff(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (not g_bRealisticAvionics or
            not(SimDriver.GetPlayerAircraft()->af->platform->IsF16() or
                SimDriver.GetPlayerAircraft()->af->platform->GetiffEnabled()))
        ...
        else if (g_bMLU)
        {
            ...
            if (not realEWSProgNum)
                realEWSProgNum = SimDriver.GetPlayerAircraft()->EWSProgNum;
...
void SimDropFlare(unsigned long, int state, void*)
{
    ...
    if (playerAC and playerAC->IsSetFlag(MOTION_OWNSHIP) and
        (state bitand KEY_DOWN))
    {
        // RV - Biker - Hack AC with IFF now can drop programmed EWS (player only)
        // if( not g_bRealisticAvionics or not SimDriver.GetPlayerAircraft()->af->platform->IsF16())
        if (not g_bRealisticAvionics or
            not(playerAC->af->platform->IsF16() or
                playerAC->af->platform->GetiffEnabled()))
        {
...
void SimHSDRangeStepUp(unsigned long, int state, void*)
{
    ...
    if (SimDriver.GetPlayerAircraft() and
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP) and
        not F4IsBadReadPtr(SimDriver.GetPlayerAircraft(),
                           sizeof(AircraftClass)) and
        SimDriver.GetPlayerAircraft()->FCC and
        not F4IsBadWritePtr(SimDriver.GetPlayerAircraft()->FCC,
                            sizeof(FireControlComputer)) and
        (state bitand KEY_DOWN)) // JB 010220 CTD
    {
        ...
        (state bitand KEY_DOWN))
    {
        if (not g_bRealisticAvionics)
        {
            ...
            else
            {
                ...
                OTWDriver.pCockpitManager->mpIcp->HandleInput(
                    AA_BUTTON, (CPButtonObject*)pButton);
        ...
        else
        {
            ...
            */
            ...
            if (SimDriver.GetPlayerAircraft()->FCC->GetMasterMode() not_eq
                FireControlComputer::Dogfight)
            {
                // Put the radar in the its default AA mode
                RadarClass* pradar = (RadarClass*)FindSensor(
                    SimDriver.GetPlayerAircraft(), SensorClass::Radar);
...
void SimStepMasterArm(unsigned long, int state, void*)
{
    if (SimDriver.GetPlayerAircraft() and
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP) and
        (state bitand KEY_DOWN) and SimDriver.GetPlayerAircraft()->Sms)
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
        {
            ...
            {
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_MASTER_ARM, 2);
            }
            else if (SimDriver.GetPlayerAircraft()->Sms->MasterArm() ==
                     SMSBaseClass::Sim)
            {
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_MASTER_ARM, 3);
            }
            else // safe
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_MASTER_ARM, 1);
        }
    }
}
...
void SimSafeMasterArm(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN) and SimDriver.GetPlayerAircraft()->Sms)
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_MASTER_ARM,
                                                         3);
    }
}
...
void SimNextAGWeapon(unsigned long val, int state, void* pButton)
{
    ...
    if (F4IsBadReadPtr(SimDriver.GetPlayerAircraft(), sizeof(AircraftClass)) or
        not SimDriver.GetPlayerAircraft()->FCC or // JB 010305 CTD
        F4IsBadReadPtr(SimDriver.GetPlayerAircraft()->FCC,
                       sizeof(FireControlComputer)) or // JB 010305 CTD
        SimDriver.GetPlayerAircraft()->FCC->GetMasterMode() ==
        ...
        (state bitand KEY_DOWN))
    {
        if (not g_bRealisticAvionics)
        {
            ...
            else
            {
                ...
                OTWDriver.pCockpitManager->mpIcp->HandleInput(
                    AG_BUTTON, (CPButtonObject*)pButton);
                MfdDisplay[0]->SetNewMode(MFDClass::FCRMode);
                MfdDisplay[1]->SetNewMode(MFDClass::SMSMode);
            }
        }
...
void SimNextNavMode(unsigned long val, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        if (OTWDriver.pCockpitManager->mpIcp->GetICPPrimaryMode() not_eq
            NAV_MODE)
        {
        ...
        else
        {
            SimICPTILS(
                val, state,
                OTWDriver.pCockpitManager->GetButtonPointer(ICP_ILS_BUTTON_ID));
        }
    }
}
...
{
    if (SimDriver.GetPlayerAircraft() not_eq NULL and
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP))
    {
        // We only want to eject if the eject key combo is held
        // for > 1 second.
        if (((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered ==
            FALSE)
        {
            if (state bitand KEY_DOWN)
            {
                ...
                if (g_bRealisticAvionics)
                {
                    if (((AircraftClass*)SimDriver.GetPlayerAircraft())
                            ->SeatArmed)
                    {
                        // Start the timer
                        ((AircraftClass*)SimDriver.GetPlayerAircraft())
                            ->ejectCountdown =
                            0.5; //THW 2004-02-17 Changed from 1.0 to 0.5 seconds
                        ((AircraftClass*)SimDriver.GetPlayerAircraft())
                            ->doEjectCountdown = TRUE;
                    }
                }
                else
                {
...
void AFBrakesOut(unsigned long, int state, void*)
{
    if (SimDriver.GetPlayerAircraft() and
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP))
    {
        if (SimDriver.GetPlayerAircraft()->af->HydraulicA() == 0)
            return;
...
void AFBrakesIn(unsigned long, int state, void*)
{
        ...
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP))
    {
        ...
        {
            SimDriver.GetPlayerAircraft()->af->speedBrake = -1.0F;
            SimDriver.GetPlayerAircraft()->speedBrakeState =
                SimDriver.GetPlayerAircraft()->af->dbrake;
...
void AFGearToggle(unsigned long, int state, void*)
{
    if (SimDriver.GetPlayerAircraft() and
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP) and
        SimDriver.GetPlayerAircraft()->af and
        (state bitand
         KEY_DOWN)) // 2002-02-15 MODIFIED BY S.G. Uncommented the ...IsSetFlag(MOTION_OWNSHIP) section and moved it before the ...af check since MOTION_OWNSHIP is only cleared when we're ejected (and we have no gears when ejected anyway)
    {
        ...
        {
            if (SimDriver.GetPlayerAircraft()->mFaults->GetFault(
                    FaultClass::gear_fault) or
...
void OTWStepNextPadlock(unsigned long, int state, void*)
{
        ...
        (SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP)))
    {
        ...
        if (FalconLocalGame->rules.GetPadlockMode() not_eq PDDisabled and
            OTWDriver.GetOTWDisplayMode() not_eq
                OTWDriverClass::ModePadlockF3 and
            OTWDriver.GetOTWDisplayMode() not_eq
...
void OTWStepMFD1(unsigned long, int state, void*)
{
        ...
        (SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP)))
    {
        ...
        //MI
        if (SimDriver.GetPlayerAircraft()->FCC and
            SimDriver.GetPlayerAircraft()->FCC->IsSOI and g_bRealisticAvionics)
        {
        ...
        (SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP)))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            if (TheHud->GetScalesSwitch() < 3)
                ...
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HUD_VAH, 1 << (2 - TheHud->GetScalesSwitch()));
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_HUD_VAH,
                                                             1);
    }
}
...
void OTWTogglePitchLadder(unsigned long, int state, void*)
{
        ...
        (SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP)))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            ...
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                COMP_3DPIT_HUD_FPM_LADD, 1 << TheHud->GetFPMSwitch());
    }
}
...
void OTWSelectOrbitMode(unsigned long, int state, void*)
{
    if (FalconLocalGame and
        SimDriver.GetPlayerAircraft()) //added SimDriver check
    {
        if (FalconLocalGame->rules.ExternalViewOn() or
            (not FalconLocalGame->rules.ExternalViewOn() and
             SimDriver.GetPlayerAircraft()->OnGround() and g_bExtViewOnGround))
        {
            if (state bitand KEY_DOWN)
            {
...
{
    if (FalconLocalGame and SimDriver.GetPlayerAircraft() and
        (FalconLocalGame->rules.ExternalViewOn() or
         not FalconLocalGame->rules.ExternalViewOn() and
             SimDriver.GetPlayerAircraft()->OnGround() and
        ...
        (SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP)))
    {
        if (TheHud)
        {
            ...
            if (OTWDriver.GetVirtualCockpit())
                ...
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_ICP_BRT_WHEEL, 1 << curColorIdx);
        }
    }
}
...
void OTWStepHudContrastDn(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (TheHud)
        {
            ...
            if (TheHud->ContWheelPos < 0.0F)
                TheHud->ContWheelPos = 0.0F;
            ...
            if (OTWDriver.GetVirtualCockpit())
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    ...
                    1 << (((int)(TheHud->ContWheelPos * 10)) + 1));
        }
    }
}
...
void OTWStepHudContrastUp(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (TheHud)
        {
            ...
            if (TheHud->ContWheelPos > 1.0F)
                TheHud->ContWheelPos = 1.0F;
...
{
    if (state bitand KEY_DOWN and
        SimDriver.GetPlayerAircraft()->IsSetFlag(
            MOTION_OWNSHIP)) // 2002-02-15 MODIFIED BY S.G. ToggleEyeFly brings you to another AC and can crash when you're ejected since your class isn't AircraftClass anymore
...
void SimDriftCO(unsigned long, int state, void*)
{
    if (state bitand KEY_DOWN and
        SimDriver.GetPlayerAircraft()->IsSetFlag(
            MOTION_OWNSHIP)) // 2002-02-15 MODIFIED BY S.G. Only valid when not ejected
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            ...
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                COMP_3DPIT_ICP_DRIFTCO, 1 << TheHud->GetDriftCOSwitch());
    }
}
...
void SimDriftCOOn(unsigned long, int state, void*)
{
            ...
            MOTION_OWNSHIP)) // 2002-02-15 MODIFIED BY S.G. Only valid when not ejected
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_ICP_DRIFTCO,
                                                         2);
    }
}
...
void SimCATSwitch(unsigned long, int state, void*)
{
    ...
    if (playerAC and playerAC->IsSetFlag(MOTION_OWNSHIP) and
        (state bitand KEY_DOWN) and playerAC->Sms)
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
        {
            if (SimDriver.GetPlayerAircraft()->af->IsSet(
                ...
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_STORES_CAT, 2);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_STORES_CAT, 1);
        }
    }
}
...
void SimCATI(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN) and SimDriver.GetPlayerAircraft()->Sms)
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_STORES_CAT,
                                                         1);
    }
}
...
void OTW1200DView(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        OTWDriver.pCockpitManager->SetActivePanel(
            100); //100 = id for 12:00 Down panel
    }
}
...
void OTW1200View(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (SimDriver.GetPlayerAircraft())
        {
            ...
            OTWDriver.pCockpitManager->SetActivePanel(
                1100); //1100 = id for 12:00 50-50 panel
    }
}
...
void SimWheelBrakes(unsigned long, int state, void*)
{
        ...
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP))
    {
        ...
        {
            if (SimLibElapsedTime - thrrevtimer <= 500 and
                SimDriver.GetPlayerAircraft()->af->thrustReverse < 2)
            {
        ...
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP))
    {
        ...
        if (state bitand KEY_DOWN and
            not SimDriver.GetPlayerAircraft()->af->IsSet(
                AirframeClass::GearBroken))
            SimDriver.GetPlayerAircraft()->af->SetFlag(
                AirframeClass::WheelBrakes);
        else
            SimDriver.GetPlayerAircraft()->af->ClearFlag(
                AirframeClass::WheelBrakes);
    }
...
void FOVToggle(unsigned long, int state, void*)
{
    if (state bitand KEY_DOWN)
    {
        ...
        //degrees when it is at 60, and will set it to 60, whenever it is not at 60;
        {
            ...
            OTWDriver.SetFOV(
                g_fNarrowFOV *
                DTR); //Wombat778 2-21-2004  Changed to g_fNarrowFov
        }
        else
        {
            narrowFOV = FALSE;
            OTWDriver.SetFOV(g_fDefaultFOV *
                             DTR); //Wombat778 10-31-2003 changed to default FOV
        }
    }
}
...
void FOVDecrease(unsigned long, int state, void*) //Wombat778 9-27-2003
{
    ...
    {
        if (OTWDriver.GetFOV() >
            (g_fMinimumFOV + g_fFOVIncrement) *
                DTR) //Added g_fMinimumFOV //Ensure that we never set the FOV to 0 or less
        {
            OTWDriver.SetFOV(OTWDriver.GetFOV() - (g_fFOVIncrement * DTR));

            if (OTWDriver.GetFOV() ==
                g_fDefaultFOV *
                    DTR) //Wombat778 10-31-2003 changed to default FOV  //set narrowFOV to false if we are back to the default FOV
                narrowFOV = FALSE;
            else
                narrowFOV = TRUE;
        }
...
void FOVIncrease(unsigned long, int state, void*) //Wombat778 9-27-2003
{
    ...
    {
        if (OTWDriver.GetFOV() <=
            (g_fMaximumFOV - g_fFOVIncrement) *
                DTR) //10/11/03 Ensure that we never set the FOV to greater than the max
        {
            narrowFOV = TRUE;
            OTWDriver.SetFOV(OTWDriver.GetFOV() + (g_fFOVIncrement * DTR));
...
void SimEmergencyJettison(unsigned long, int state, void*)
{
    if (SimDriver.GetPlayerAircraft() not_eq NULL and
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP))
    {
        if (not SimDriver.GetPlayerAircraft()->Sms or
            not SimDriver.GetPlayerAircraft()->Sms->drawable)
            return;
        ...
        if (g_bRealisticAvionics)
        {
            ...
            if (SimDriver.GetPlayerAircraft()->OnGround() and
                SimDriver.GetPlayerAircraft()->Sms and
                not SimDriver.GetPlayerAircraft()->Sms->GndJett)
            {
                return;
            }
        }
            ...
            if (state bitand KEY_DOWN)
            {
                ...
                if (g_bRealisticAvionics)
                {
                    ...
                    if (MfdDisplay[1]->GetCurMode() not_eq MFDClass::SMSMode or
                        SimDriver.GetPlayerAircraft()
                                ->Sms->drawable->DisplayMode() not_eq
                            SmsDrawable::EmergJet)
                    {
                        MfdDisplay[0]->EmergStoreMode =
                            MfdDisplay[0]->CurMode();
                        MfdDisplay[1]->EmergStoreMode =
                            MfdDisplay[1]->CurMode();
                        MfdDisplay[1]->SetNewMode(MFDClass::SMSMode);
                        ...
                        {
                            SimDriver.GetPlayerAircraft()
                                ->Sms->drawable->EmergStoreMode =
                                SimDriver.GetPlayerAircraft()
                                    ->Sms->drawable->DisplayMode();

                            SimDriver.GetPlayerAircraft()
                                ->Sms->drawable->SetDisplayMode(
                                    SmsDrawable::EmergJet);
                        }
                    }
...
void SimECMStandby(unsigned long, int state,
                   void*) //Switches ECM off.  Map to HOTAS CMS hat "right"
{
    if (not g_bRealisticAvionics) // not intended for anything other than realistic mode
        return;
...
void SimECMConsent(unsigned long, int state,
                   void*) //Switches ECM on.  Map to HOTAS CMS hat "down"
{
    if (not g_bRealisticAvionics) // not intended for anything other than realistic mode
        return;
...
void SimHsiCourseInc(unsigned long, int state, void*)
{
    if (state bitand KEY_DOWN and
        SimDriver.GetPlayerAircraft()->IsSetFlag(
            MOTION_OWNSHIP)) // 2002-02-15 MODIFIED BY S.G. Any cockpit stuff is valid only if the player hasn't ejected... Vince, you should know better than this ;-)
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_HSI_COURSE,
                                                         val);
    }
}
...
void SimHsiHeadingInc(unsigned long, int state, void*)
{
            ...
            MOTION_OWNSHIP)) // 2002-02-15 MODIFIED BY S.G. Any cockpit stuff is valid only if the player hasn't ejected... Vince, you should know better than this ;-)
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_HSI_HEADING,
                                                         val);
    }
}
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
        {
            if (gNavigationSys->GetInstrumentMode() ==
                NavigationSystem::ILS_TACAN)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HSI_MODE, 1);
            else if (gNavigationSys->GetInstrumentMode() ==
                     NavigationSystem::TACAN)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HSI_MODE, 2);
            else if (gNavigationSys->GetInstrumentMode() ==
                     NavigationSystem::NAV)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HSI_MODE, 4);
            else if (gNavigationSys->GetInstrumentMode() ==
                     NavigationSystem::ILS_NAV)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HSI_MODE, 8);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HSI_MODE, 1);
        }
...
void SimHSIIlsTcn(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        SimDriver.GetPlayerAircraft()->FCC->SetMasterMode(
            FireControlComputer::ILS);

        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_HSI_MODE,
                                                         1);
    }
}
...
void SimCBEOSB_1L(unsigned long, int state, void*)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        SimDriver.GetPlayerAircraft()->IsSetFlag(
            MOTION_OWNSHIP)) // 2002-02-15 MODIFIED BY S.G. Any cockpit stuff is valid only if the player hasn't ejected...
    {
...
void SimCBEOSB_1R(unsigned long, int state, void*)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        SimDriver.GetPlayerAircraft()->IsSetFlag(
            MOTION_OWNSHIP)) // 2002-02-15 MODIFIED BY S.G. Any cockpit stuff is valid only if the player hasn't ejected...
    {
        OTWDriver.pCockpitManager->mMiscStates.SetMFDButtonState(
            1, 1,
            1); //Wombat778 4-12-04 changed from 0,1,1 to 1,1,1.  This must have been a bug.
...
void SimICPTILS(unsigned long, int state, void* pButton)
{
    if (not g_bRealisticAvionics)
    {
        ...
        {
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                ILS_BUTTON, (CPButtonObject*)pButton);

            // If we're in NAV mode, update the FCC/HUD mode
            if (OTWDriver.pCockpitManager->mpIcp->GetICPPrimaryMode() ==
                NAV_MODE)
            {
                if (OTWDriver.pCockpitManager->mpIcp->GetICPSecondaryMode() ==
                    ILS_MODE)
                {
                    SimDriver.GetPlayerAircraft()->FCC->SetMasterMode(
                        FireControlComputer::ILS);
    ...
    else
    {
        ...
        else
        {
            ...
            if (SimDriver.GetPlayerAircraft() and
                SimDriver.GetPlayerAircraft()->FCC and
                SimDriver.GetPlayerAircraft()->FCC->IsNavMasterMode())
            {
                if (OTWDriver.pCockpitManager->mpIcp->GetICPSecondaryMode() ==
                        ILS_MODE and
                    (gNavigationSys->GetInstrumentMode() ==
                         NavigationSystem::ILS_NAV or
                     gNavigationSys->GetInstrumentMode() ==
                         NavigationSystem::ILS_TACAN))
                {
                    SimDriver.GetPlayerAircraft()->FCC->SetMasterMode(
...
void SimICPALOW(unsigned long, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        OTWDriver.pCockpitManager->mpIcp->HandleInput(ALOW_BUTTON,
...
void SimICPFAck(unsigned long, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        OTWDriver.pCockpitManager->mpIcp->HandleInput(FACK_BUTTON,
                                                      (CPButtonObject*)pButton);
...
void SimICPPrevious(unsigned long, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        OTWDriver.pCockpitManager->mpIcp->HandleInput(PREV_BUTTON,
                                                      (CPButtonObject*)pButton);

        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_ICP_NEXT,
                                                         4);
    }
}
...
void SimICPNext(unsigned long, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        OTWDriver.pCockpitManager->mpIcp->HandleInput(NEXT_BUTTON,
                                                      (CPButtonObject*)pButton);
...
void SimICPLink(unsigned long, int state, void* pButton)
{
    ...
    if (state bitand KEY_DOWN and playerAC and
        playerAC->IsSetFlag(MOTION_OWNSHIP) and not playerAC->ejectTriggered)
    {
        // test code
        FalconDLinkMessage* pmsg;
        pmsg = new FalconDLinkMessage(SimDriver.GetPlayerAircraft()->Id(),
                                      FalconLocalGame);
        ...
        OTWDriver.pCockpitManager->mpIcp->HandleInput(DLINK_BUTTON,
                                                      (CPButtonObject*)pButton);
    }
}
...
void SimICPCrus(unsigned long, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        OTWDriver.pCockpitManager->mpIcp->HandleInput(CRUS_BUTTON,
                                                      (CPButtonObject*)pButton);
    }
}
...
void SimICPStpt(unsigned long, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        OTWDriver.pCockpitManager->mpIcp->HandleInput(STPT_BUTTON,
                                                      (CPButtonObject*)pButton);
    }
}
...
void SimICPMark(unsigned long, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        OTWDriver.pCockpitManager->mpIcp->HandleInput(MARK_BUTTON,
                                                      (CPButtonObject*)pButton);
    }
}
...
void SimICPEnter(unsigned long, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        OTWDriver.pCockpitManager->mpIcp->HandleInput(ENTR_BUTTON,
                                                      (CPButtonObject*)pButton);
    }
}
...
void SimICPCom1(unsigned long, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        OTWDriver.pCockpitManager->mpIcp->HandleInput(COMM1_BUTTON,
                                                      (CPButtonObject*)pButton);
    }
}
...
void SimICPCom2(unsigned long, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        OTWDriver.pCockpitManager->mpIcp->HandleInput(COMM2_BUTTON,
                                                      (CPButtonObject*)pButton);
    }
}
...
void SimICPNav(unsigned long, int state, void* pButton)
{
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        ...
        OTWDriver.pCockpitManager->mpIcp->HandleInput(NAV_BUTTON,
                                                      (CPButtonObject*)pButton);

        if (F4IsBadReadPtr(SimDriver.GetPlayerAircraft(),
                           sizeof(AircraftClass)) or // JB 010317 CTD
            not SimDriver.GetPlayerAircraft()->FCC or // JB 010307 CTD
            F4IsBadReadPtr(SimDriver.GetPlayerAircraft()->FCC,
                           sizeof(FireControlComputer)) or // JB 010307 CTD
            SimDriver.GetPlayerAircraft()->FCC->GetMasterMode() ==
                FireControlComputer::MissileOverride or
            SimDriver.GetPlayerAircraft()->FCC->GetMasterMode() ==
                FireControlComputer::Dogfight)
    ...
    else
    {
        ...
        if (state bitand KEY_DOWN and not playerAC->ejectTriggered)
        {
            ...
            if (OTWDriver.pCockpitManager->mpIcp->IsICPSet(ICPClass::MODE_A_A))
            {
                ...
                playerAC->FCC->SetMasterMode(
                    playerAC->FCC->GetLastNavMasterMode()); //ASSOCIATOR

                //passes our puttonstate to the ICP
                OTWDriver.pCockpitManager->mpIcp->HandleInput(
                    NAV_BUTTON, (CPButtonObject*)pButton);
...
void SimICPAA1(unsigned long, int state, void* pButton)
{
        ...
        not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
    {
        ...
        if (SimDriver.GetPlayerAircraft() and
            SimDriver.GetPlayerAircraft()->FCC and
            (SimDriver.GetPlayerAircraft()->FCC->GetMasterMode() ==
                 FireControlComputer::Dogfight or
             SimDriver.GetPlayerAircraft()->FCC->GetMasterMode() ==
                 FireControlComputer::MissileOverride))
            return;

        OTWDriver.pCockpitManager->mpIcp->HandleInput(AA_BUTTON,
                                                      (CPButtonObject*)pButton);
        SimDriver.GetPlayerAircraft()->FCC->SetMasterMode(
            FireControlComputer::AAGun);
...
{
    if (not SimDriver.GetPlayerAircraft() or
        not SimDriver.GetPlayerAircraft()->IsSetFlag(
            MOTION_OWNSHIP)) // 2002-02-15 ADDED BY S.G. Do it once here and removed the corresponding line from below
        return;
    ...
    if (not g_bRealisticAvionics)
    {
            ...
            not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
        {
            ...
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                AG_BUTTON, (CPButtonObject*)pButton);
            SimDriver.GetPlayerAircraft()->FCC->SetMasterMode(
                FireControlComputer::AirGroundBomb);
    ...
    else
    {
        ...
        if (SimDriver.GetPlayerAircraft()
                ->Sms) //JPO CTDfix //Set our display mode
            SimDriver.GetPlayerAircraft()->Sms->drawable->SetDisplayMode(
                SmsDrawable::Wpn);
            ...
            not((AircraftClass*)SimDriver.GetPlayerAircraft())->ejectTriggered)
        {
            ...
            if (OTWDriver.pCockpitManager->mpIcp->IsICPSet(ICPClass::MODE_A_G))
            {
                ...
                SimDriver.GetPlayerAircraft()->FCC->SetMasterMode(
                    SimDriver.GetPlayerAircraft()
                        ->FCC->GetLastNavMasterMode()); //ASSOCIATOR
...
void SimICPAG1(unsigned long, int state, void* pButton)
{
    if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
        ...
        SimDriver.GetPlayerAircraft()->Sms)
    {
        //MI 3/1/2002 if we're in DF or MRM, don't do anything
        if (SimDriver.GetPlayerAircraft() and
            SimDriver.GetPlayerAircraft()->FCC and
        ...
        OTWDriver.pCockpitManager->mpIcp->HandleInput(AG_BUTTON,
                                                      (CPButtonObject*)pButton);
        SimDriver.GetPlayerAircraft()->FCC->SetMasterMode(
            FireControlComputer::AirGroundBomb);
            ...
            MOTION_OWNSHIP)) // 2002-02-15 MODIFIED BY S.G. Any cockpit stuff is valid only if the player hasn't ejected...
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
        {
            if (TheHud->GetDEDSwitch() == HudClass::PFL_DATA)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HUD_DED_PFL, 2);
            else if (TheHud->GetDEDSwitch() == HudClass::DED_DATA)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HUD_DED_PFL, 4);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HUD_DED_PFL, 1);
        }
    }
}
...
void SimHUDDEDOff(unsigned long, int state, void*)
{
        ...
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_HUD_DED_PFL,
                                                         1);
    }
}
            ...
            MOTION_OWNSHIP)) // 2002-02-15 MODIFIED BY S.G. Any cockpit stuff is valid only if the player hasn't ejected...
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
        {
            if (TheHud->GetVelocitySwitch() == HudClass::CAS)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HUD_VELOCITY, 4);
            else if (TheHud->GetVelocitySwitch() == HudClass::TAS)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HUD_VELOCITY, 2);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HUD_VELOCITY, 1);
        }
    }
}
            ...
            MOTION_OWNSHIP)) // 2002-02-15 MODIFIED BY S.G. Any cockpit stuff is valid only if the player hasn't ejected...
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
        {
            if (TheHud->GetRadarSwitch() == HudClass::ALT_RADAR)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HUD_RAL_BARO, 4);
            else if (TheHud->GetRadarSwitch() == HudClass::BARO)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HUD_RAL_BARO, 2);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_HUD_RAL_BARO, 1);
        }
    }
}
...
void SimHUDBrightness(unsigned long, int state, void*)
{
            ...
            MOTION_OWNSHIP)) // 2002-02-15 MODIFIED BY S.G. Any cockpit stuff is valid only if the player hasn't ejected...
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                COMP_3DPIT_HUD_DAY_NITE,
                1 << (TheHud->GetBrightnessSwitch() - 1));
    }
}
...
void SimCycleLeftAuxComDigit(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetDOFangle(
                COMP_3DPIT_TACAN_LEFT, val * 0.6283F); // radians per digit
    }
}
...
void SimDecLeftAuxComDigit(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetDOFangle(COMP_3DPIT_TACAN_LEFT,
                                                       val * 0.6283F);
    }
}
...
void SimCycleCenterAuxComDigit(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetDOFangle(COMP_3DPIT_TACAN_CENTER,
                                                       val * 0.6283F);
    }
}
...
void SimCycleRightAuxComDigit(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetDOFangle(COMP_3DPIT_TACAN_RIGHT,
                                                       val * 0.6283F);
    }
}
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            if (gNavigationSys->GetTacanBand(NavigationSystem::AUXCOMM) ==
                TacanList::X)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_TACAN_BAND, 1);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_TACAN_BAND, 2);
    }
}
...
void SimToggleAuxComMaster(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        gNavigationSys
            ->ToggleUHFSrc(); // MD -- 20031121: added here to match reality

        if (OTWDriver.GetVirtualCockpit())
            if (gNavigationSys->GetControlSrc() == NavigationSystem::AUXCOMM)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_AUX_COMM_SRC, 1);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_AUX_COMM_SRC, 2);
    }
}
...
void IncreaseAlow(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (TheHud and TheHud->lowAltWarning >= 1000.0F)
        {
            ...
            TheHud->lowAltWarning =
                min(max(0.0F, TheHud->lowAltWarning), 99999.0F);
        }
        else if (TheHud)
        {
...
void SimThrottleIdleDetent(unsigned long, int state, void*)
{
    ...
    {
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_IDLE_DETENT,
                                                         2);
        ...
        if (SimDriver.GetPlayerAircraft()->af->GetNumberEngines() == 2)
        {
            ...
            case UserStickInputs.Left_Engine:
                ...
                {
                    SimDriver.GetPlayerAircraft()->af->SetFlag(
                        AirframeClass::EngineStopped);
                    SimDriver.GetPlayerAircraft()->af->SetFlag(
                        AirframeClass::EngineOff);
                }
                else if (SimDriver.GetPlayerAircraft()->af->rpm >= 0.20f)
                {
                    ...
                    SimDriver.GetPlayerAircraft()->af->ClearFlag(
                        AirframeClass::EngineOff);
                    SimDriver.GetPlayerAircraft()->mFaults->ClearFault(
                        FaultClass::eng_fault, FaultClass::fl_out);
                }
            ...
            case UserStickInputs.Right_Engine:
                ...
                {
                    SimDriver.GetPlayerAircraft()->af->SetEngineFlag(
                        AirframeClass::EngineStopped2);
                    SimDriver.GetPlayerAircraft()->af->SetFlag(
                        AirframeClass::EngineOff2);
                }
                else if (SimDriver.GetPlayerAircraft()->af->rpm2 >= 0.20f)
                {
                    // engine light
                    SimDriver.GetPlayerAircraft()->af->ClearEngineFlag(
                        AirframeClass::EngineStopped2);
                    SimDriver.GetPlayerAircraft()->af->ClearFlag(
                        AirframeClass::EngineOff);
                    SimDriver.GetPlayerAircraft()->mFaults->ClearFault(
                        FaultClass::eng2_fault, FaultClass::fl_out);
                }

                break;

            case UserStickInputs.Both_Engines:
                if (SimDriver.GetPlayerAircraft()->af->engine1Throttle <
                        0.1f and
...
void AFRudderTrimLeft(unsigned long, int state, void*)
{
    if (SimDriver.GetPlayerAircraft() and
        SimDriver.GetPlayerAircraft()->IsSetFlag(
            MOTION_OWNSHIP)) // 2002-02-15 ADDED BY S.G. Moved outside the two if statement
    {
        //MI additions
        if (g_bRealisticAvionics)
        {
...
void AFAileronTrimRight(unsigned long, int state, void*)
{
            ...
            MOTION_OWNSHIP)) // 2002-02-15 ADDED BY S.G. Moved outside the two if statement
    {
        ...
        {
            if (SimDriver.GetPlayerAircraft() and
                SimDriver.GetPlayerAircraft()->TrimAPDisc)
                return;
        }
...
void SimICPIFF(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            ...
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                IFF_BUTTON, (CPButtonObject*)pButton);
        }
    }
}
...
void SimICPLIST(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            //Player pushed the button, tell it to our ICP class
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                LIST_BUTTON, (CPButtonObject*)pButton);
        }
    }
}
...
void SimICPTHREE(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            //Player pushed the button, tell it to our ICP class
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                THREE_BUTTON, (CPButtonObject*)pButton);
        }
    }
}
...
void SimICPSIX(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            //Player pushed the button, tell it to our ICP class
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                SIX_BUTTON, (CPButtonObject*)pButton);
        }
        else
        {
            if (state bitand KEY_DOWN and SimDriver.GetPlayerAircraft() and
            ...
            {
                OTWDriver.pCockpitManager->mpIcp->HandleInput(
                    DLINK_BUTTON, (CPButtonObject*)pButton);
            }
        }
    }
}
...
void SimICPEIGHT(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            //Player pushed the button, tell it to our ICP class
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                EIGHT_BUTTON, (CPButtonObject*)pButton);
        }
    }
}
...
void SimICPNINE(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            //Player pushed the button, tell it to our ICP class
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                NINE_BUTTON, (CPButtonObject*)pButton);
        }
    }
}
...
void SimICPZERO(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            //Player pushed the button, tell it to our ICP class
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                ZERO_BUTTON, (CPButtonObject*)pButton);
        }
    }
}
...
void SimICPResetDED(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            //Player pushed the button, tell it to our ICP class
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                CNI_MODE, (CPButtonObject*)pButton);

            if (OTWDriver.GetVirtualCockpit())
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_ICP_DED,
                                                             8);
        }
    }
    else
...
void SimICPDEDUP(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            //Player pushed the button, tell it to our ICP class
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                UP_BUTTON, (CPButtonObject*)pButton);
...
void SimICPDEDDOWN(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            //Player pushed the button, tell it to our ICP class
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                DOWN_BUTTON, (CPButtonObject*)pButton);
...
void SimICPDEDSEQ(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            //Player pushed the button, tell it to our ICP class
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                SEQ_BUTTON, (CPButtonObject*)pButton);
...
void SimICPCLEAR(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            //Player pushed the button, tell it to our ICP class
            OTWDriver.pCockpitManager->mpIcp->HandleInput(
                CLEAR_BUTTON, (CPButtonObject*)pButton);
        }
    }
}
...
void SimRALTSTDBY(unsigned long, int state, void* pButton)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_RALT_PWR,
                                                         2);
    }
}
...
{
    if (theLantirn and SimDriver.GetPlayerAircraft() and
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP) and
        (state bitand KEY_DOWN))
    {
...
void SimSMSPower(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            ...
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                COMP_3DPIT_SMS_PWR, SimDriver.GetPlayerAircraft()->HasPower(
...
void SimDLPower(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            ...
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                COMP_3DPIT_DL_PWR, 1 << SimDriver.GetPlayerAircraft()->HasPower(
...
void SimRightHptPower(unsigned long val, int state, void*)
{
    if (SimDriver.GetPlayerAircraft() and
        ...
        (state bitand KEY_DOWN))
    {
        SimDriver.GetPlayerAircraft()->PowerToggle(
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            if (SimDriver.GetPlayerAircraft()->HasPower(
                ...
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_LEFT_HPT_PWR, 2);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_LEFT_HPT_PWR, 1);
    }
}
...
void SimFCRPower(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                COMP_3DPIT_FCR_PWR, SimDriver.GetPlayerAircraft()->HasPower(
...
void SimHUDPower(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                COMP_3DPIT_HUD_PWR, SimDriver.GetPlayerAircraft()->HasPower(
...
void SimToggleRealisticAvionics(unsigned long val, int state, void*)
{
    if (SimDriver.GetPlayerAircraft() and
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP) and
        (state bitand
         KEY_DOWN)) // 2002-02-10 MODIFIED BY S.G. Removed ; at the end of the if statement
...
void SimIncFuelSwitch(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                COMP_3DPIT_FUEL_QTY,
                (SimDriver.GetPlayerAircraft()->af->GetFuelSwitch() + 1));
    }
}
...
void SimFuelSwitchTest(unsigned long val, int state, void*)
{
    ...
    if (SimDriver.GetPlayerAircraft() and
        ...
        (state bitand KEY_DOWN))
    {
        SimDriver.GetPlayerAircraft()->af->SetFuelSwitch(
            AirframeClass::FS_TEST);

        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_FUEL_QTY,
                                                         32);
    }
}
...
void SimIncFuelPump(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_REFUEL_PUMP,
                                                         val);
    }
}
...
void SimToggleMasterFuel(unsigned long val, int state, void*)
{
    if (SimDriver.GetPlayerAircraft() and
        ...
        (state bitand KEY_DOWN))
    {
        SimDriver.GetPlayerAircraft()->af->ToggleEngineFlag(
        ...
        if (OTWDriver.GetVirtualCockpit())
            if (SimDriver.GetPlayerAircraft()->af->IsEngineFlag(
                    AirframeClass::MasterFuelOff))
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_REFUEL_MSTR, 1);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_REFUEL_MSTR, 2);
    }
}
...
void SimMasterFuelOn(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_REFUEL_MSTR,
                                                         2);
    }
}
...
void SimLandingLightOn(unsigned long val, int state, void*)
{
    ...
    if (pac and pac->IsSetFlag(MOTION_OWNSHIP) and (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_LAND_LIGHT,
                                                         2);
    }
}
...
void SimParkingBrakeToggle(unsigned long val, int state, void*)
{
    ...
    if (OTWDriver.GetVirtualCockpit())
    {
        if (SimDriver.GetPlayerAircraft()->af->PBON == TRUE)
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_PARK_BRAKE,
                                                         2);
        else
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_PARK_BRAKE,
                                                         1);
    ...
    )
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
        {
            if (SimDriver.GetPlayerAircraft()->af->IsSet(AirframeClass::Hook))
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_HOOK,
                                                             2);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_HOOK,
                                                             1);
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            if (SimDriver.GetPlayerAircraft()->FCC->LaserArm)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_LASER_ARM, 2);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_LASER_ARM, 1);
    }
}
...
void SimLaserArmOn(unsigned long val, int state, void*)
{
    ...
    if (SimDriver.GetPlayerAircraft() and
        ...
        (state bitand KEY_DOWN))
    {
        if (SimDriver.GetPlayerAircraft()->Sms->MasterArm() not_eq
            SMSBaseClass::MasterArmState::Arm)
            return;
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_LASER_ARM,
                                                         2);
    }
}
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            if (SimDriver.GetPlayerAircraft()->af->IsEngineFlag(
                    AirframeClass::FuelDoorOpen))
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_REFUEL_DOOR, 2);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_REFUEL_DOOR, 1);
    }
}
...
void SimFuelDoorOpen(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_REFUEL_DOOR,
                                                         2);
    }
}
...
void SimRightAPSwitch(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (g_bRealisticAvionics)
        {
            ...
            if (SimDriver.GetPlayerAircraft()->IsOn(
                    AircraftClass::AltHold)) //up
            {
                //from Alt hold to Att hold
                SimDriver.GetPlayerAircraft()->ClearAPFlag(
                    AircraftClass::AltHold);
                //can't be both
                SimDriver.GetPlayerAircraft()->SetAPFlag(
                    AircraftClass::AttHold); //down
                ...
                if ((not SimDriver.GetPlayerAircraft()->IsOn(
                        AircraftClass::StrgSel)) and
                    (not SimDriver.GetPlayerAircraft()->IsOn(
                        AircraftClass::StrgSel)))
                {
                    SimDriver.GetPlayerAircraft()->SetAPFlag(
                        AircraftClass::
                            RollHold); // needed in case this is the first time and switch is still in default position
            ...
            else if (SimDriver.GetPlayerAircraft()->IsOn(
                         AircraftClass::AttHold))
            {
                //all off
                SimDriver.GetPlayerAircraft()->ClearAPFlag(
                    AircraftClass::AltHold);
                ...
                SimDriver.GetPlayerAircraft()->SetAutopilot(
                    AircraftClass::APOff);
                //take the juice
            ...
            if (OTWDriver.GetVirtualCockpit())
            {
                ...
                if (SimDriver.GetPlayerAircraft()->IsOn(AircraftClass::AltHold))
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RT_AP_SW, 4);
                // Right switch down position
                else if (SimDriver.GetPlayerAircraft()->IsOn(
                             AircraftClass::AttHold))
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RT_AP_SW, 1);
...
void SimRightAPUp(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if ((not SimDriver.GetPlayerAircraft()->IsOn(
                AircraftClass::StrgSel)) and
            (not SimDriver.GetPlayerAircraft()->IsOn(AircraftClass::HDGSel)))
        {
            SimDriver.GetPlayerAircraft()->SetAPFlag(
                AircraftClass::
                    RollHold); // needed in case this is the first time and switch is still in default position
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_RT_AP_SW,
                                                         2);
    }
}
...
void SimLeftAPSwitch(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (SimDriver.GetPlayerAircraft()->IsOn(AircraftClass::RollHold))
        {
            ...
            SimDriver.GetPlayerAircraft()->SetAPFlag(
                AircraftClass::StrgSel); //waypoint AP
        }
        //down position
        ...
        if ((SimDriver.GetPlayerAircraft()->IsOn(AircraftClass::AttHold)) or
            (SimDriver.GetPlayerAircraft()->IsOn(AircraftClass::AltHold)))
        {
            if (SimDriver.GetPlayerAircraft()->IsOn(AircraftClass::RollHold))
            {
                SimDriver.GetPlayerAircraft()->SetNewRoll();
        ...
        if (OTWDriver.GetVirtualCockpit())
        {
            ...
            if (SimDriver.GetPlayerAircraft()->IsOn(AircraftClass::RollHold))
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_LT_AP_SW, 4);
            // Left switch down position
            else if (SimDriver.GetPlayerAircraft()->IsOn(
                         AircraftClass::StrgSel))
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_LT_AP_SW, 1);
            // Left switch up position
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_LT_AP_SW, 2);
        }
    }
}
...
void SimLeftAPUp(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_LT_AP_SW,
                                                         2);
    }
}
...
void SimAPOverride(unsigned long val, int state, void*)
{
        ...
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP))
    {
        if (state bitand KEY_DOWN)
        {
            ...
            if (SimDriver.GetPlayerAircraft()->AutopilotType() not_eq
                AircraftClass::APOff)
                SimDriver.GetPlayerAircraft()->lastapType =
                    SimDriver.GetPlayerAircraft()->AutopilotType();
        ...
        else
        {
            ...
            if (SimDriver.GetPlayerAircraft()->lastapType not_eq
                AircraftClass::LantirnAP) // JPO - lantirn stays
            {
                SimDriver.GetPlayerAircraft()->SetAPParameters();

                if (SimDriver.GetPlayerAircraft()->IsOn(
                        AircraftClass::RollHold))
                    SimDriver.GetPlayerAircraft()->SetNewRoll();
...
void SimReticlePri(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_HUD_RETICLE,
                                                         2);
    }
}
...
void SimInstrumentLight(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        default:
        case AircraftClass::LT_OFF:
            SimDriver.GetPlayerAircraft()->SetInstrumentLight(
                AircraftClass::LT_LOW);
            break;

        case AircraftClass::LT_LOW:
            SimDriver.GetPlayerAircraft()->SetInstrumentLight(
                AircraftClass::LT_NORMAL);
            break;

        case AircraftClass::LT_NORMAL:
            SimDriver.GetPlayerAircraft()->SetInstrumentLight(
                AircraftClass::LT_OFF);
            break;
        }
        ...
        TheColorBank.PitLightLevel =
            (int)SimDriver.GetPlayerAircraft()->GetInstrumentLight();
...
void SimSpotLight(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        case AircraftClass::LT_OFF:
            ...
            SimDriver.GetPlayerAircraft()->PowerOn(
                AircraftClass::SpotLightPower);
            break;

        case AircraftClass::LT_LOW:
            SimDriver.GetPlayerAircraft()->SetSpotLight(
                AircraftClass::LT_NORMAL);
            SimDriver.GetPlayerAircraft()->PowerOn(
                AircraftClass::SpotLightPower);
            break;
        ...
        case AircraftClass::LT_NORMAL:
            ...
            SimDriver.GetPlayerAircraft()->PowerOff(
                AircraftClass::SpotLightPower);
            break;
        }
    ...
    if (pac and pac->IsSetFlag(MOTION_OWNSHIP))
    {
        if (state bitand KEY_DOWN)
        {
            if (g_bRealisticAvionics)
            {
                ...
                {
                    if (pac->Sms->curWeapon and
                        not((MissileClass*)pac->Sms->curWeapon.get())
                               ->Covered and
    ...
    if (pac and pac->IsSetFlag(MOTION_OWNSHIP))
    {
        if (state bitand KEY_DOWN)
        {
            if (g_bRealisticAvionics and not g_bMLU)
            {
                ...
                {
                    MissileClass* m = static_cast<MissileClass*>(
                        pac->Sms->GetCurrentWeapon());
                    m->HOC = not m->HOC;
                }
            }
            ...
            if (g_bRealisticAvionics)
            {
                ...
                if (theRadar and theRadar->IsSOI())
                {
                        ...
                        theRadar->GetRadarMode() == RadarClass::ACM_10x60)
                    {
                        ...
                        theRadar
                            ->SelectACMVertical(); //Cobra BMS bug fix 01/29/05
                    }
                    else if (theRadar->GetRadarMode() ==
                                 RadarClass::ACM_30x20 or
                             theRadar->GetRadarMode() == RadarClass::ACM_SLEW or
                    ...
                    else if (theRadar->GetRadarMode() == RadarClass::TWS)
                    {
                        ...
                        if (theRadar->CurrentTarget() or
                            theRadar->twsTrackDirectory())
                        {
                            //First, drop our track
                            SimDriver.GetPlayerAircraft()->FCC->dropTrackCmd =
                                TRUE;
                    ...
                    else if (theRadar->GetRadarMode() == RadarClass::SAM and
                ...
                if (SimDriver.GetPlayerAircraft()->FCC and
                    SimDriver.GetPlayerAircraft()->FCC->IsSOI)
                {
                    SimDriver.GetPlayerAircraft()->FCC->HSDDesignate = -1;
                    return;
                }
...
void SimTMSRight(unsigned long val, int state, void*)
{
        ...
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP))
    {
        if (g_bMLU)
        {
            if (state bitand KEY_DOWN and (not g_bMLU or not tmstimer or
                                           SimLibElapsedTime - tmstimer < 500))
            {
                if (not tmstimer)
                    tmstimer = SimLibElapsedTime;
            }
            else
            {
                if (SimLibElapsedTime - tmstimer < 500 or not g_bMLU)
                {
                    if (g_bRealisticAvionics)
                    {
                        ...
                        {
                            //ACM Modes
                            if (theRadar->GetRadarMode() ==
                                    RadarClass::ACM_30x20 or
                                theRadar->GetRadarMode() ==
                                    RadarClass::ACM_SLEW or
                                theRadar->GetRadarMode() ==
                                    RadarClass::ACM_BORE or
                                theRadar->GetRadarMode() ==
                                    RadarClass::ACM_10x60)
                            {
                                SimDriver.GetPlayerAircraft()
                                    ->FCC->dropTrackCmd = FALSE;
                                theRadar->SelectACM30x20();
...
void SimEWSRWROn(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_EWS_RWR_PWR,
                                                         2);
    }
}
...
void SimEWSJammerOn(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_EWS_JMR_PWR,
                                                         2);
    }
}
...
void SimEWSPGMInc(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_EWS_MODE,
                                                         val);
    }
}
...
void SimEWSModeStby(unsigned long val, int state, void*)
{
    ...
    if (SimDriver.GetPlayerAircraft() and
        ...
        (state bitand KEY_DOWN))
    {
        SimDriver.GetPlayerAircraft()->SetPGM(
            AircraftClass::EWSPGMSwitch::Stby);
        PlayerRwrClass* theRwr = (PlayerRwrClass*)FindSensor(
            SimDriver.GetPlayerAircraft(), SensorClass::RWR);
...
void SimEWSModeMan(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if ((SimDriver.GetPlayerAircraft()->EWSPGM() ==
             AircraftClass::EWSPGMSwitch::Off) or
            (SimDriver.GetPlayerAircraft()->EWSPGM() ==
             AircraftClass::EWSPGMSwitch::Stby))
        {
            PlayerRwrClass* theRwr = (PlayerRwrClass*)FindSensor(
                SimDriver.GetPlayerAircraft(), SensorClass::RWR);
...
void SimEWSModeSemi(unsigned long val, int state, void*)
{
    ...
    {
        SimDriver.GetPlayerAircraft()->SetPGM(
            AircraftClass::EWSPGMSwitch::Semi);
...
void SimEWSModeAuto(unsigned long val, int state, void*)
{
    ...
    {
        SimDriver.GetPlayerAircraft()->SetPGM(
            AircraftClass::EWSPGMSwitch::Auto);
...
void SimEWSProgInc(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_EWS_PROG,
                                                         val);
    }
}
...
void SimEWSProgOne(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        SimDriver.GetPlayerAircraft()->SetEWSProg(
            0); // First knob position is zero
...
void SimEWSProgFour(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        SimDriver.GetPlayerAircraft()->SetEWSProg(
            3); // Fourth knob position is three
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            if (OTWDriver.GetVirtualCockpit())
                ...
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_MAIN_PWR,
                    1 << SimDriver.GetPlayerAircraft()->MainPowerOn());
    }
}
...
void SimMainPowerOff(unsigned long val, int state, void*)
{
    ...
    if (SimDriver.GetPlayerAircraft() and
        ...
        (state bitand KEY_DOWN))
    {
        SimDriver.GetPlayerAircraft()->SetMainPower(
            AircraftClass::MainPowerOff);

        if (not g_bINS)
        {
            SimDriver.GetPlayerAircraft()->INSOff(
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_MAIN_PWR,
                                                         1);
    }
}
        ...
        if (g_bRealisticAvionics)
        {
            ...
            if (OTWDriver.GetVirtualCockpit())
            {
                if (SimDriver.GetPlayerAircraft()->playBetty)
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_VMS_PWR, 1);
                else
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_VMS_PWR, 2);
            }
        }
    }
}
            ...
            if (g_bRealisticAvionics)
            {
                ...
                if (theRadar)
                {
                    ...
                    else
                    {
                        ...
                        if (SimDriver.GetPlayerAircraft()->lastapType ==
                            AircraftClass::LantirnAP)
                            SimDriver.GetPlayerAircraft()->SetAutopilot(
                                SimDriver.GetPlayerAircraft()->lastapType);
...
void SimRFNorm(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (state bitand KEY_DOWN)
        {
            ...
            if (SimDriver.GetPlayerAircraft()->RFState ==
                2) //restore AP if the RF was in "silent"
                if (SimDriver.GetPlayerAircraft()->lastapType ==
                    AircraftClass::LantirnAP)
                    SimDriver.GetPlayerAircraft()->SetAutopilot(
                        SimDriver.GetPlayerAircraft()->lastapType);
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (theRwr)
        {
            ...
            if (OTWDriver.GetVirtualCockpit())
                if (theRwr->IsOn())
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_PWR, 2);
                else
                    OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                        COMP_3DPIT_RWR_PWR, 1);
        }
    }
}
...
void SimDropProgrammed(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (g_bRealisticAvionics)
        {
            ...
            if (not SimDriver.GetPlayerAircraft()->HasPower(
                    AircraftClass::UFCPower) or
...
void SimCMSDown(unsigned long val, int state, void* cb)
{
    ...
    SimECMConsent(val, state,
                  cb); // specifically Consent ("enable"), not SimECMOn (toggle)
}
        ...
        if (state bitand KEY_DOWN)
        {
            if (g_bRealisticAvionics)
            {
                ...
                if (pac->Sms->curWeaponType == wtAgm65 and pac->Sms->curWeapon)
                {
                    HasMavs = TRUE;
                    mavDisplay =
                        (MaverickDisplayClass*)((MissileClass*)pac->Sms
                                                    ->GetCurrentWeapon())
                            ->display;
                }
                ...
                else if (SimDriver.GetPlayerAircraft()->FCC and
                         SimDriver.GetPlayerAircraft()->FCC->IsSOI)
...
void SimGndJettOff(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (SimDriver.GetPlayerAircraft()->Sms)
        {
            ...
            if (SimDriver.GetPlayerAircraft()
                    ->OnGround()) // Jammer is disabled when on the ground unless GND JETT is enabled
                SimDriver.GetPlayerAircraft()->UnSetFlag(ECM_ON);
        }
    }
}
...
void SimToggleExtLights(unsigned long, int state, void*)
{
    ...
    if (ac->IsSetFlag(MOTION_OWNSHIP) and (state bitand KEY_DOWN) and
        ac->IsComplex())
    {
        if (ac->IsAcStatusBitsSet(AircraftClass::ACSTATUS_EXT_LIGHTS))
        {
            ...
            {
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_EXT_LITE_MSTR, 1);
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_ANTI_COLL, 1);
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_EXT_WING, 1);
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_EXT_FUSELAGE, 1);
            }
        }
        else
        {
            ...
            ac->SetAcStatusBits(AircraftClass::ACSTATUS_EXT_LIGHTS bitor
                                AircraftClass::ACSTATUS_EXT_NAVLIGHTS |
                                AircraftClass::ACSTATUS_EXT_NAVLIGHTSFLASH bitor
                                AircraftClass::ACSTATUS_EXT_TAILSTROBE);
            ...
            {
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_EXT_LITE_MSTR, 2);
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_ANTI_COLL, 2);
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_EXT_WING, 2);
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_EXT_FUSELAGE, 2);
            }
        }
    }
}
...
void SimExtlMasterNorm(unsigned long val, int state, void*)
{
    ...
    if (SimDriver.GetPlayerAircraft() and
        ...
        SimDriver.GetPlayerAircraft()->af->IsSet(AirframeClass::HasComplexGear))
    {
        SimDriver.GetPlayerAircraft()->ExtlOn(
            AircraftClass::ExtlLightFlags::Extl_Main_Power);
...
void SimExtlMasterOff(unsigned long val, int state, void*)
{
    ...
    if (SimDriver.GetPlayerAircraft() and
        ...
        SimDriver.GetPlayerAircraft()->af->IsSet(AirframeClass::HasComplexGear))
    {
        SimDriver.GetPlayerAircraft()->ExtlOff(
            AircraftClass::ExtlLightFlags::Extl_Main_Power);
...
void SimAntiCollOn(unsigned long val, int state, void*)
{
        ...
        SimDriver.GetPlayerAircraft()->af->IsSet(AirframeClass::HasComplexGear))
    {
        ...
        SimDriver.GetPlayerAircraft()->SetAcStatusBits(
            AircraftClass::ACSTATUS_EXT_TAILSTROBE);
        SimDriver.GetPlayerAircraft()->ExtlOn(
            AircraftClass::ExtlLightFlags::Extl_Anti_Coll);

        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_ANTI_COLL,
                                                         2);
    }
}
...
void SimWingLightBrt(unsigned long val, int state, void*)
{
        ...
        SimDriver.GetPlayerAircraft()->af->IsSet(AirframeClass::HasComplexGear))
    {
        SimDriver.GetPlayerAircraft()->SetAcStatusBits(
            AircraftClass::ACSTATUS_EXT_NAVLIGHTS);
        SimDriver.GetPlayerAircraft()->ExtlOn(
            AircraftClass::ExtlLightFlags::Extl_Wing_Tail);

        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_EXT_WING,
                                                         4);
    }
}
...
void SimLightsSteady(unsigned long val, int state, void*)
{
    ...
    {
        SimDriver.GetPlayerAircraft()->ExtlOff(
            AircraftClass::ExtlLightFlags::Extl_Flash);

        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_EXT_FLASH,
                                                         1);
    }
}
// flashing aka. ON
...
void SimAVTRSwitch(unsigned long val, int state, void* pButton)
{
    ...
    {
        //off-auto-on
        if (SimDriver.GetPlayerAircraft()->AVTRState(
                AircraftClass::AVTRStateFlags::AVTR_OFF))
        {
            SimDriver.GetPlayerAircraft()->AVTROn(
                AircraftClass::AVTRStateFlags::AVTR_AUTO);
            SimDriver.GetPlayerAircraft()->AVTROff(
                AircraftClass::AVTRStateFlags::AVTR_OFF);
            SimDriver.GetPlayerAircraft()->AVTROff(
                AircraftClass::AVTRStateFlags::AVTR_ON); //just in case

            if (OTWDriver.GetVirtualCockpit())
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_AVTR_SW,
                                                             2);
        }
        ...
        {
            SimDriver.GetPlayerAircraft()->AVTROn(
                AircraftClass::AVTRStateFlags::AVTR_ON);
            SimDriver.GetPlayerAircraft()->AVTROff(
                AircraftClass::AVTRStateFlags::AVTR_AUTO);
            SimDriver.GetPlayerAircraft()->AVTROff(
                AircraftClass::AVTRStateFlags::AVTR_OFF); //just in case
        ...
        else
        {
            ...
            SimDriver.GetPlayerAircraft()->AVTROff(
                AircraftClass::AVTRStateFlags::AVTR_ON);
            SimDriver.GetPlayerAircraft()->AVTROff(
                AircraftClass::AVTRStateFlags::AVTR_AUTO); //just in case
...
void SimAutoAVTR(unsigned long val, int state, void* pButton)
{
        ...
        (SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP)))
    {
        ...
        if (not SimDriver.GetPlayerAircraft()->AVTRState(
                AircraftClass::AVTRStateFlags::AVTR_AUTO))
            return;
...
void SimIFFPower(unsigned long val, int state, void*)
{
        ...
        (SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP)))
    {
                ...
                AircraftClass::IFFPower))
        {
            ...
            if (OTWDriver.GetVirtualCockpit())
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_IFF_PWR,
                                                             2);
        }
        else
        {
...
void SimIFFIn(unsigned long val, int state, void*)
{
        ...
        (SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP)))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_IFF_QUERY,
                                                         2);
    }
    else
    {
...
void SimINSInc(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (SimDriver.GetPlayerAircraft()->INSState(
                AircraftClass::INS_PowerOff))
        {
            SimDriver.GetPlayerAircraft()->SwitchINSToAlign();

            if (OTWDriver.GetVirtualCockpit())
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_INS_MODE, 2);
        }
...
void SimINSOff(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_INS_MODE,
                                                         1);
    }
}
...
void SimLEFLockSwitch(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (SimDriver.GetPlayerAircraft()->LEFLocked)
        {
            ...
            if (SimDriver.GetPlayerAircraft()->mFaults and
                not SimDriver.GetPlayerAircraft()->LEFState(
                    AircraftClass::LEFSASYNCH))
                SimDriver.GetPlayerAircraft()->mFaults->ClearFault(lef_fault);

            if (OTWDriver.GetVirtualCockpit())
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(
                    COMP_3DPIT_LEF_FLAPS, 1);
        }
        else
        {
...
void SimLEFLock(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_LEF_FLAPS,
                                                         2);
    }
}
...
void SimAltFlapsNorm(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_ALT_FLAPS,
                                                         1);
    }
}
...
void SimTrimAPDisc(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (SimDriver.GetPlayerAircraft()->TrimAPDisc)
        {
            ...
            if (OTWDriver.GetVirtualCockpit())
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_TRIM_AP,
                                                             2);
        }
        else
        {
...
void SimTrimAPDISC(unsigned long val, int state,
                   void*) // case *is* significant for command names
{
    if (not g_bRealisticAvionics)
        return;
...
void SimTrimNoseUp(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetDOFangle(
                COMP_3DPIT_TRIM_PITCH, cockpitFlightData.TrimPitch + 0.5f);
    }
}
...
void SimTrimYawLeft(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetDOFangle(
                COMP_3DPIT_TRIM_YAW, cockpitFlightData.TrimYaw - 2.0f);
    }
}
...
void SimTrimRollLeft(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetDOFangle(
                COMP_3DPIT_TRIM_ROLL, cockpitFlightData.TrimRoll - 0.5f);
    }
}
...
void SimTriggerFirstDetent(unsigned long val, int state, void*)
{
        ...
        SimDriver.GetPlayerAircraft()->IsSetFlag(MOTION_OWNSHIP))
    {
        ...
        if (state bitand KEY_DOWN)
        {
            ...
            if (laserPod and SimDriver.GetPlayerAircraft()->FCC->LaserArm and
                SimDriver.GetPlayerAircraft()->FCC->GetMasterMode() ==
                    FireControlComputer::AirGroundLaser)
            {
        ...
        if (state bitand KEY_DOWN)
        {
            ...
            if (SimDriver.GetPlayerAircraft()->FCC and
                (SimDriver.GetPlayerAircraft()->FCC->GetMasterMode() ==
                 ...
                 SimDriver.GetPlayerAircraft()->FCC->GetMasterMode() ==
                     FireControlComputer::MissileOverride or
                 SimDriver.GetPlayerAircraft()->FCC->GetSubMode() ==
                     FireControlComputer::STRAF or
...
void AFFullFlap(unsigned long, int state, void*)
{
    ...
    //10 = AUTO, 20 = HALF, 30 = FULL
    {
        ...
        {
            if (SimDriver.GetPlayerAircraft()->af->flapPos not_eq 10 and
        ...
        (state bitand KEY_DOWN))
    {
        if (SimDriver.GetPlayerAircraft()->af->HasDragChute())
            ...
            case AirframeClass::DRAGC_STOWED:
                SimDriver.GetPlayerAircraft()->af->dragChute =
                    AirframeClass::DRAGC_DEPLOYED;
                break;

            case AirframeClass::DRAGC_DEPLOYED:
            case AirframeClass::DRAGC_TRAILING:
                SimDriver.GetPlayerAircraft()->af->dragChute =
                    AirframeClass::DRAGC_JETTISONNED;
                break;
            }

        if (SimDriver.GetPlayerAircraft()->af->dragChute ==
            AirframeClass::DRAGC_STOWED)
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_DRAGCHUTE,
                                                         1);
        else
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_DRAGCHUTE,
                                                         2);
...
void SimCursorEnable(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (SimDriver.GetPlayerAircraft()->Sms)
        {
            if (SimDriver.GetPlayerAircraft()->Sms->curWeaponType == wtAgm65 and
                SimDriver.GetPlayerAircraft()->Sms->curWeapon)
                SimDriver.GetPlayerAircraft()->Sms->StepMavSubMode();
            else if (SimDriver.GetPlayerAircraft()->Sms->curWeaponType ==
                         wtAim9 or
                     SimDriver.GetPlayerAircraft()->Sms->curWeaponType ==
                         wtAim120)
            {
                if (SimDriver.GetPlayerAircraft()->FCC)
                    SimDriver.GetPlayerAircraft()->FCC->missileSlaveCmd = TRUE;
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (OTWDriver.GetVirtualCockpit())
            if (SimDriver.GetPlayerAircraft()->af->canopyState == true)
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_CANOPY,
                                                             2);
            else
                OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_CANOPY,
                                                             1);
...
void SimStepComm1VolumeUp(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        // MLR 1/29/2004 -
        PlayerOptions.GroupVol[COM1_SOUND_GROUP] = (int)(RESCALE(
            OTWDriver.pCockpitManager->mpIcp->Comm1Volume, 7, 0, -2000, 0));

        if (OTWDriver.pCockpitManager->mpIcp->Comm1Volume == 8)
            PlayerOptions.GroupVol[COM1_SOUND_GROUP] = -10000;
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_COMM1_VOL,
                                                         vall);
    }
}
...
void SimStepComm2VolumeUp(unsigned long val, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        // MLR 1/29/2004 -
        PlayerOptions.GroupVol[COM2_SOUND_GROUP] = (int)(RESCALE(
            OTWDriver.pCockpitManager->mpIcp->Comm2Volume, 7, 0, -2000, 0));

        if (OTWDriver.pCockpitManager->mpIcp->Comm2Volume == 8)
            PlayerOptions.GroupVol[COM2_SOUND_GROUP] = -10000;
        ...
        if (OTWDriver.GetVirtualCockpit())
            OTWDriver.GetVirtualCockpit()->SetSwitchMask(COMP_3DPIT_COMM2_VOL,
                                                         vall);
    }
}
...
void SimSymWheelUp(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (TheHud)
        {
            ...
            if ((TheHud->SymWheelPos >= 0.1F) and
                (not SimDriver.GetPlayerAircraft()->HasPower(
                    AircraftClass::HUDPower)))
            {
                SimDriver.GetPlayerAircraft()->PowerOn(AircraftClass::HUDPower);
...
void SimSymWheelDn(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        if (TheHud)
        {
            ...
            if (TheHud->SymWheelPos < 0.0F)
                TheHud->SymWheelPos = 0.0f;
...
void SimRandomError(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        */
        ...
        {
            SimDriver.GetPlayerAircraft()->af->HydrBreak(
                AirframeClass::HYDR_A_SYSTEM);
        }

        if (rand() % 100 < 20) // 20% failure chance of B system
        {
            SimDriver.GetPlayerAircraft()->af->HydrBreak(
                AirframeClass::HYDR_B_SYSTEM);
        }

        // also break the generators now and then
        if (rand() % 7 == 1)
            SimDriver.GetPlayerAircraft()->af->GeneratorBreak(
                AirframeClass::GenStdby);
...
void SimToggleCockpit(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        if (curPanel->mIdNum == 5000)
        {
            if (SimDriver.GetPlayerAircraft()->WideView)
            {
                if (OTWDriver.pCockpitManager->SetActivePanel(
                        1100)) //Wombat778 4-13-04 Check for the presence of the panel before changing variables
                {
            ...
            {
                if (OTWDriver.pCockpitManager->SetActivePanel(
                        91100)) //Wombat778 4-13-04 Check for the presence of the panel before changing variables
                {
        ...
        else if (SimDriver.GetPlayerAircraft()->WideView)
        {
            if (OTWDriver.pCockpitManager->SetActivePanel(
                    curPanel->mIdNum -
                    90000)) //Wombat778 4-13-04 Check for the presence of the panel before changing variables
            {
                SimDriver.GetPlayerAircraft()->WideView = FALSE;
...
void SimFuelDump(unsigned long val, int state, void*)
{
    ...
    {
        float fueltodump =
            max((SimDriver.GetPlayerAircraft()->af->Fuel() +
                 SimDriver.GetPlayerAircraft()->af->ExternalFuel()) /
                    15.0f,
                100);
...
void SimRangeKnobDown(unsigned long val, int state, void*)
{
    ...
    if (SimDriver.GetPlayerAircraft() and
        ...
        (state bitand KEY_DOWN))
    {
        RadarDopplerClass* theRadard = (RadarDopplerClass*)FindSensor(
            SimDriver.GetPlayerAircraft(), SensorClass::Radar);
...
void Profiler_HistoryBackFast(unsigned long, int state,
                              void*) // Retro 22May2004
{
#ifdef Prof_ENABLED
...
void ToggleTIR(unsigned long, int state, void*)
{
    if (state bitand KEY_DOWN)
    {
        if ((g_bEnableTrackIR) and
            (PlayerOptions.Get3dTrackIR() == true)) // TIR running?
        {
            if (OTWDriver.IsHeadTracking())
            {
                OTWDriver.SetHeadTracking(
                    FALSE); // 3D pit mouselook On and TIR is Off.
                g_bTrackIRon = false; // Tell trackir
            }
            else
            {
                OTWDriver.SetHeadTracking(
                    TRUE); // 3D pit mouselook Off and TIR is On.
...
void SimToggleAltView(unsigned long, int state, void*)
{
        ...
        (state bitand KEY_DOWN))
    {
        ...
        else
        {
            //Switch it on
            OTWDriver.pCockpitManager->SetActivePanel(
                OTWDriver.pCockpitManager->AltPanel());
        }
    }
}
