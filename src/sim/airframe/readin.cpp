...
int AirframeClass::ReadData(int idx)
{
             ...
             aeroDataset[idx].inputData[AeroDataSet::InternalFuel])
    {
        fuel = aeroDataset[idx].inputData[AeroDataSet::InternalFuel];
        externalFuel =
            initialFuel - aeroDataset[idx].inputData[AeroDataSet::InternalFuel];
    }
    else
    {
        fuel = initialFuel;
    ...
    curMaxStoreSpeed = maxVcas =
        aeroDataset[idx]
            .inputData[AeroDataSet::MaxVcas]; //me123 addet curMaxVcas
    cornerVcas = aeroDataset[idx].inputData[AeroDataSet::CornerVcas];

    gear = new GearData[FloatToInt32(
        aeroDataset[idx].inputData[AeroDataSet::NumGear])];

    for (int i = 0; i < aeroDataset[idx].inputData[AeroDataSet::NumGear]; i++)
    {
        gear[i].strength = 100.0F; //how many hitpoints it has left
        gear[i].vel =
            0.0F; //at what rate is it currently compressing/extending in ft/s
...
void ReadData(float *inputData, SimlibFileClass *inputFile)
{
    ...
    {
        inputData[AeroDataSet::NosGearX + i * 4] =
            (float)atof(inputFile->GetNext());
        inputData[AeroDataSet::NosGearY + i * 4] =
            (float)atof(inputFile->GetNext());
        inputData[AeroDataSet::NosGearZ + i * 4] =
            (float)atof(inputFile->GetNext());
        inputData[AeroDataSet::NosGearRng + i * 4] =
            (float)atof(inputFile->GetNext());
    }
...
void ReadAllAirframeData(void)
{
    ...
    */
    ...
    for (i = 0; i < num_sets; i++)
    {
        ...
        aeroDataset[i].auxaeroData =
            AirframeAuxAeroRead(inputFile); // JB 010714
        inputFile->Close();
        delete inputFile;
        inputFile = NULL;
    }
...
AeroData *AirframeAeroRead(SimlibFileClass *inputFile)
{
    ...
    /*---------------------------------*/
#ifdef USE_SH_POOLS
    aeroData->clift = (float *)MemAllocPtr(
        gReadInMemPool, sizeof(float) * numMach * numAlpha, 0);
#else
    ...
    for (mach = 0; mach < numMach; mach++)
    {
        ...
        {
            aeroData->clift[mach * numAlpha + alpha] =
                (float)atof(inputFile->GetNext());
        }
    }
    ...
    /*---------------------------------*/
#ifdef USE_SH_POOLS
    aeroData->cdrag = (float *)MemAllocPtr(
        gReadInMemPool, sizeof(float) * numMach * numAlpha, 0);
#else
    ...
    for (mach = 0; mach < numMach; mach++)
    {
        for (alpha = 0; alpha < numAlpha; alpha++)
        {
            aeroData->cdrag[mach * numAlpha + alpha] =
                (float)atof(inputFile->GetNext()) * 1.5F;
        }
    }
    ...
    /*------------------------------*/
#ifdef USE_SH_POOLS
    aeroData->cy = (float *)MemAllocPtr(gReadInMemPool,
                                        sizeof(float) * numMach * numAlpha, 0);
#else
    ...
    for (mach = 0; mach < numMach; mach++)
    {
        for (alpha = 0; alpha < numAlpha; alpha++)
        {
            aeroData->cy[mach * numAlpha + alpha] =
                (float)atof(inputFile->GetNext());
        }
    }
...
static const InputDataDesc AuxAeroDataDesc[] = {
    ...
    {"jfsSpoolUpLimit", InputDataDesc::ID_FLOAT, OFFSET(jfsSpoolUpLimit),
     "0.25"},
    {"lightupSpoolRate", InputDataDesc::ID_FLOAT, OFFSET(lightupSpoolRate),
     "10"},
    {"flameoutSpoolRate", InputDataDesc::ID_FLOAT, OFFSET(flameoutSpoolRate),
     "5"},
    {"jfsRechargeTime", InputDataDesc::ID_FLOAT, OFFSET(jfsRechargeTime), "60"},
    {"jfsMinRechargeRpm", InputDataDesc::ID_FLOAT, OFFSET(jfsMinRechargeRpm),
     "0.12"},
    ...
    {"DeepStallEngineStall", InputDataDesc::ID_INT,
     OFFSET(DeepStallEngineStall), "0"}, //Need bool
    {"engineDamageStopThreshold", InputDataDesc::ID_INT,
     OFFSET(engineDamageStopThreshold),
     "13"}, // 2002-04-11 ADDED BY S.G. Externalized the 13% chance of engine stop
    {"engineDamageNoRestartThreshold", InputDataDesc::ID_INT,
     OFFSET(engineDamageNoRestartThreshold),
     "50"}, // 2002-04-11 ADDED BY S.G. Externalized the 50% chance of engine not able to restart
    {"engineDamageHitThreshold", InputDataDesc::ID_INT,
     OFFSET(engineDamageHitThreshold),
     "0"}, // 2002-04-11 ADDED BY S.G. Externalized the hitChance > 0 chance of engine shutting down
    ...
    {"gearPitchFactor", InputDataDesc::ID_FLOAT, OFFSET(gearPitchFactor),
     "0.1"},
    ...
    {"tefMaxAngle", InputDataDesc::ID_FLOAT, OFFSET(tefMaxAngle),
     "20"}, // divide angle of TEF by this to get amount of influence on CL and CD
    {"hasFlapperons", InputDataDesc::ID_INT, OFFSET(hasFlapperons),
     "1"}, // has flapperons are opposed to ailerons bitand flaps
    {"hasTef", InputDataDesc::ID_INT, OFFSET(hasTef),
     "2"}, // has TEF 0 - no, 1 manual, 2 aoa
    {"CLtefFactor", InputDataDesc::ID_FLOAT, OFFSET(CLtefFactor),
     "0.05"}, // how much the TEF affect the CL
    {"CDtefFactor", InputDataDesc::ID_FLOAT, OFFSET(CDtefFactor),
     "0.05"}, // how much the TEF affect the CD
    {"tefNStages", InputDataDesc::ID_INT, OFFSET(tefNStages),
     "2"}, // number of stages of TEF
    {"tefTakeoff", InputDataDesc::ID_FLOAT, OFFSET(tefTakeOff),
     "0"}, // Tef angle for takeoff
    {"tefRate", InputDataDesc::ID_FLOAT, OFFSET(tefRate),
     "5"}, // Tef rate of change
    {"lefRate", InputDataDesc::ID_FLOAT, OFFSET(lefRate),
     "5"}, // Lef rate of change
    {"hasLef", InputDataDesc::ID_INT, OFFSET(hasLef), "2"}, // has LEF
    {"lefGround", InputDataDesc::ID_FLOAT, OFFSET(lefGround),
     "-2"}, // lef position on ground
    {"lefMaxAngle", InputDataDesc::ID_FLOAT, OFFSET(lefMaxAngle),
     "20"}, // max angle of LEF by this to get amount of influence on CL and CD
    {"lefMaxMach", InputDataDesc::ID_FLOAT, OFFSET(lefMaxMach),
     "0.4"}, // LEF stops at this
    {"lefNStages", InputDataDesc::ID_INT, OFFSET(lefNStages),
     "1"}, // number of stages of LEF
    {"CDlefFactor", InputDataDesc::ID_FLOAT, OFFSET(CDlefFactor),
     "0.0"}, // how much the LEF affect the CD
    {"CDSPDBFactor", InputDataDesc::ID_FLOAT, OFFSET(CDSPDBFactor),
     "0.08"}, // how much the speed brake affect drag
    {"CDLDGFactor", InputDataDesc::ID_FLOAT, OFFSET(CDLDGFactor),
     "0.06"}, // how much the landing gear affects drag
    {"flapGearRelative", InputDataDesc::ID_INT, OFFSET(flapGearRelative),
     "1"}, //flaps only work with gear (or tef extend)
    {"maxFlapVcas", InputDataDesc::ID_FLOAT, OFFSET(maxFlapVcas),
     "370"}, // falps fully retract at
    {"flapVcasRange", InputDataDesc::ID_FLOAT, OFFSET(flapVcasRange),
     "130"}, // what range of Vcas flaps work over
    {"area2Span", InputDataDesc::ID_FLOAT, OFFSET(area2Span),
     "0.1066"}, // used to convert life area into span
    {"rudderMaxAngle", InputDataDesc::ID_FLOAT, OFFSET(rudderMaxAngle),
     "20"}, // max angle of rudder
    {"aileronMaxAngle", InputDataDesc::ID_FLOAT, OFFSET(aileronMaxAngle),
     "20"}, // max angle of aileron
    {"elevonMaxAngle", InputDataDesc::ID_FLOAT, OFFSET(elevonMaxAngle),
     "20"}, // max angle of elevon
    {"airbrakeMaxAngle", InputDataDesc::ID_FLOAT, OFFSET(airbrakeMaxAngle),
     "60"}, // max angle of airbrake
    {"hasSwingWing", InputDataDesc::ID_INT, OFFSET(hasSwingWing),
     "0"}, // has swing wing capability
    {"isComplex", InputDataDesc::ID_INT, OFFSET(isComplex),
     "0"}, // has complex model
    {"landingAOA", InputDataDesc::ID_FLOAT, OFFSET(landingAOA), "12.5"},
    {"dragChuteCd", InputDataDesc::ID_FLOAT, OFFSET(dragChuteCd), "0"},
    {"dragChuteMaxSpeed", InputDataDesc::ID_FLOAT, OFFSET(dragChuteMaxSpeed),
     "170"},
    ...
    // fuel related
    {"fuelFlowFactorNormal", InputDataDesc::ID_FLOAT,
     OFFSET(fuelFlowFactorNormal), "0.25"},
    {"fuelFlowFactorAb", InputDataDesc::ID_FLOAT, OFFSET(fuelFlowFactorAb),
     "0.65"},
    ...
    {"engine1Location", InputDataDesc::ID_VECTOR, OFFSET(engineLocation[0]),
     "0 0 0"},
    {"engine2Location", InputDataDesc::ID_VECTOR, OFFSET(engineLocation[1]),
     "0 0 0"},
    {"engine3Location", InputDataDesc::ID_VECTOR, OFFSET(engineLocation[2]),
     "0 0 0"},
    {"engine4Location", InputDataDesc::ID_VECTOR, OFFSET(engineLocation[3]),
     "0 0 0"},
    {"engine5Location", InputDataDesc::ID_VECTOR, OFFSET(engineLocation[4]),
     "0 0 0"},
    {"engine6Location", InputDataDesc::ID_VECTOR, OFFSET(engineLocation[5]),
     "0 0 0"},
    {"engine7Location", InputDataDesc::ID_VECTOR, OFFSET(engineLocation[6]),
     "0 0 0"},
    {"engine8Location", InputDataDesc::ID_VECTOR, OFFSET(engineLocation[7]),
     "0 0 0"},
    {"engineSmokes", InputDataDesc::ID_INT, OFFSET(engineSmokes), "0"},
    {"wingTipLocation", InputDataDesc::ID_VECTOR, OFFSET(wingTipLocation),
     "0 0 0"},
    {"vortex1Location", InputDataDesc::ID_VECTOR, OFFSET(vortex1Location),
     "0 0 0"},
    {"vortex2Location", InputDataDesc::ID_VECTOR, OFFSET(vortex2Location),
     "0 0 0"},
    {"vortexPS1Location", InputDataDesc::ID_VECTOR, OFFSET(vortexPS1Location),
     "0 0 0"},
    {"vortexPS2Location", InputDataDesc::ID_VECTOR, OFFSET(vortexPS2Location),
     "0 0 0"},
    {"vortexPS3Location", InputDataDesc::ID_VECTOR, OFFSET(vortexPS3Location),
     "0 0 0"},
    {"largeVortex", InputDataDesc::ID_INT, OFFSET(largeVortex), "0"},
    {"vortexAOALimit", InputDataDesc::ID_VECTOR, OFFSET(vortexAOALimit),
     "29.5"},
    {"refuelLocation", InputDataDesc::ID_VECTOR, OFFSET(refuelLocation),
     "0 0 0"},
    ...
    {"hp1SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[0]),
     "-1 -1 0"},
    {"hp2SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[1]),
     "-1 -1 0"},
    {"hp3SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[2]),
     "-1 -1 0"},
    {"hp4SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[3]),
     "-1 -1 0"},
    {"hp5SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[4]),
     "-1 -1 0"},
    {"hp6SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[5]),
     "-1 -1 0"},
    {"hp7SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[6]),
     "-1 -1 0"},
    {"hp8SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[7]),
     "-1 -1 0"},
    {"hp9SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[8]),
     "-1 -1 0"},
    {"hp10SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[9]),
     "-1 -1 0"},
    {"hp11SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[10]),
     "-1 -1 0"},
    {"hp12SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[11]),
     "-1 -1 0"},
    {"hp13SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[12]),
     "-1 -1 0"},
    {"hp14SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[13]),
     "-1 -1 0"},
    {"hp15SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[14]),
     "-1 -1 0"},
    {"hp16SwitchDofRate", InputDataDesc::ID_VECTOR, OFFSET(hpSwitchDofRate[15]),
     "-1 -1 0"},

    {"gunSwitchDofRate", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(gunSwitchDofRate), "-1 -1 0"},
    ...
    {"sndSpdBrakeStart", InputDataDesc::ID_INT, OFFSET(sndSpdBrakeStart),
     "141"},
    ...
    {"sndBBChaffFlareLow", InputDataDesc::ID_INT, OFFSET(sndBBChaffFlareLow),
     "184"},
    {"sndBBFlare", InputDataDesc::ID_INT, OFFSET(sndBBFlare), "138"},
    {"sndBBChaffFlare", InputDataDesc::ID_INT, OFFSET(sndBBChaffFlare), "183"},
    {"sndBBChaffFlareOut", InputDataDesc::ID_INT, OFFSET(sndBBChaffFlareOut),
     "185"},
    ...
    {"sndGearCloseStart", InputDataDesc::ID_INT, OFFSET(sndGearCloseStart),
     "147"},
    {"sndGearCloseEnd", InputDataDesc::ID_INT, OFFSET(sndGearCloseEnd), "146"},
    {"sndGearOpenStart", InputDataDesc::ID_INT, OFFSET(sndGearOpenStart),
     "149"},
    {"sndGearOpenEnd", InputDataDesc::ID_INT, OFFSET(sndGearOpenEnd), "150"},
    {"sndGearLoop", InputDataDesc::ID_INT, OFFSET(sndGearLoop), "148"},
    // MLR 2/15/2004 -
    {"sndCanopyOpenStart", InputDataDesc::ID_INT, OFFSET(sndCanopyOpenStart),
     "271"},
    {"sndCanopyOpenEnd", InputDataDesc::ID_INT, OFFSET(sndCanopyOpenEnd),
     "272"},
    {"sndCanopyCloseStart", InputDataDesc::ID_INT, OFFSET(sndCanopyCloseStart),
     "273"},
    ...
    {"startGroundAvoidCheck", InputDataDesc::ID_FLOAT,
     OFFSET(startGroundAvoidCheck),
     "3000.0"}, // 2002-04-17 MN only do ground avoid check when we are closer than this distance to the ground
    {"limitPstick", InputDataDesc::ID_INT, OFFSET(limitPstick),
     "1"}, //Cobra - changed default to 1 // 2002-04-17 MN method of pullup; 1 = use old code (G force limited), 0 = use full pull (pStick = 1.0f)
    ...
    {"refuelAltitude", InputDataDesc::ID_FLOAT, OFFSET(refuelAltitude),
     "22500.0"},
    {"decelDistance", InputDataDesc::ID_FLOAT, OFFSET(decelDistance), "1000.0"},
    {"followRate", InputDataDesc::ID_FLOAT, OFFSET(followRate),
     "20.0"}, // original was 10, 20 brings some better results
    {"desiredClosureFactor", InputDataDesc::ID_FLOAT,
     OFFSET(desiredClosureFactor), "0.25"}, // original was hardcoded to 0.25
    // { "IL78Factor", InputDataDesc::ID_FLOAT, OFFSET(IL78Factor), "0.0"}, // IL78 range to basket modifier (to fix "GivingGas" flagging)
    {"longLeg", InputDataDesc::ID_FLOAT, OFFSET(longLeg),
     "100.0"}, // 60 nm long leg of tanker track pattern
    {"shortLeg", InputDataDesc::ID_FLOAT, OFFSET(shortLeg),
     "25.0"}, // 25nm short leg of tanker track pattern
    {"refuelRate", InputDataDesc::ID_FLOAT, OFFSET(refuelRate),
     "50.0"}, // 50 lbs per second
    {"AIBoomDistance", InputDataDesc::ID_FLOAT, OFFSET(AIBoomDistance),
     "50.0"}, // distance to boom at which we will put the AI on the boom (hack...)

    // 18NOV03 - FRB - Tanker specific info - What type of service available (boom or drogue)
    {"nBooms", InputDataDesc::ID_INT, OFFSET(numBooms),
     "1"}, // 18NOV03 - FRB - Number of booms (never more than one) 0 = no boom sevice  >=1 = boom.
    {"BoomRFPos", InputDataDesc::ID_VECTOR, OFFSET(BoomRFPos),
     "0 0 0"}, // 12DEC03 - FRB - a/c 0,0,0 refueling position adjustments for different booms boom
    {"BoomStoredAngle", InputDataDesc::ID_FLOAT, OFFSET(BoomStoredAngle),
     "14"}, // 12DEC03 - FRB - Angle of boom in stored osition (deg. + = up)
    {"nDrogues", InputDataDesc::ID_INT, OFFSET(numDrogues),
     "0"}, // 18NOV03 - FRB - Number of drogues  0 = no drogue service  >0 = drogue service and number of stations.
    {"activeDrogue", InputDataDesc::ID_INT, OFFSET(activeDrogue),
     "1"}, // 18NOV03 - FRB - Number of drogue Slot used for refueling.
    {"DrogueExt", InputDataDesc::ID_FLOAT, OFFSET(DrogueExt),
     "40"}, // 29NOV03 - FRB - Drogue extension length (feet).
    {"DrogueRFPos", InputDataDesc::ID_VECTOR, OFFSET(DrogueRFPos),
     "-60 0 0"}, // 12DEC03 - FRB - a/c 0,0,0 refueling position relative to drogue pack (Slot)
    ...
    {"refuelDrogueSlot0", InputDataDesc::ID_INT, OFFSET(refuelDrogueSlot[0]),
     "0"},
    {"refuelDrogueSlot1", InputDataDesc::ID_INT, OFFSET(refuelDrogueSlot[1]),
     "1"},
    {"refuelDrogueSlot2", InputDataDesc::ID_INT, OFFSET(refuelDrogueSlot[2]),
     "2"},
    {"tankerType", InputDataDesc::ID_INT, OFFSET(tankerType), "6"},


    // 2002-03-12 MN fuel handling stuff
    {"BingoReturnDistance", InputDataDesc::ID_FLOAT,
     OFFSET(BingoReturnDistance), "0.0"},
    ...
    {"maxRippleCount", InputDataDesc::ID_INT, OFFSET(maxRippleCount),
     "19"}, // F-16's max ripple count
    {"largePark", InputDataDesc::ID_INT, OFFSET(largePark),
     "0"}, // requires large parking space

    // RV - I-Hawk - Does this plane has advanced HTS?
    {"HtsAble", InputDataDesc::ID_INT, OFFSET(HtsAble), "0"},

    //MI TFR
    {"Has_TFR", InputDataDesc::ID_INT, OFFSET(Has_TFR),
     "0"}, //does this plane have TFR?
    {"PID_K", InputDataDesc::ID_FLOAT, OFFSET(PID_K),
     "0.5"}, //Proportianal gain in TFR PID pitch controler.
    {"PID_KI", InputDataDesc::ID_FLOAT, OFFSET(PID_KI),
     "0.0"}, //Intergral gain in TFR PID pitch controler
    {"PID_KD", InputDataDesc::ID_FLOAT, OFFSET(PID_KD),
     "0.39"}, //Differential gain in TFR PID pitch controler
    {"TFR_LimitMX", InputDataDesc::ID_INT, OFFSET(TFR_LimitMX),
     "0"}, //Limit PID Integrator internal value so it doesn't get stuck in exteme.
    {"TFR_Corner", InputDataDesc::ID_FLOAT, OFFSET(TFR_Corner),
     "350.0"}, //Corner speed used in TFR calculations
    {"TFR_Gain", InputDataDesc::ID_FLOAT, OFFSET(TFR_Gain),
     "0.010"}, //Gain for calculating pitch error based on alt. difference
    {"EVA_Gain", InputDataDesc::ID_FLOAT, OFFSET(EVA_Gain),
     "5.0"}, //Pitch setpoint gain in EVA (evade) code
    {"TFR_MaxRoll", InputDataDesc::ID_FLOAT, OFFSET(TFR_MaxRoll),
     "20.0"}, //Do not pull the stick in TFR if roll exceeds this value
    {"TFR_SoftG", InputDataDesc::ID_FLOAT, OFFSET(TFR_SoftG),
     "2.0"}, //Max TFR G pull in soft mode
    {"TFR_MedG", InputDataDesc::ID_FLOAT, OFFSET(TFR_MedG),
     "4.0"}, //Max TFR G pull in medium mode
    {"TFR_HardG", InputDataDesc::ID_FLOAT, OFFSET(TFR_HardG),
     "6.0"}, //Max TFR G pull in hard mode
    {"TFR_Clearance", InputDataDesc::ID_FLOAT, OFFSET(TFR_Clearance),
     "20.0"}, //Minimum clearance above the top of any obstacle [ft]
    {"SlowPercent", InputDataDesc::ID_FLOAT, OFFSET(SlowPercent),
     "0.0"}, //Flash SLOW when airspeed is lower then this percentage of corner speed
    {"TFR_lookAhead", InputDataDesc::ID_FLOAT, OFFSET(TFR_lookAhead),
     "4000.0"}, //Distance from ground directly under a/c used to measure ground inclination [ft]
    {"EVA1_SoftFactor", InputDataDesc::ID_FLOAT, OFFSET(EVA1_SoftFactor),
     "0.6"}, //Turnradius multiplier to get safe distance from ground for FLY_UP in SLOW
    {"EVA2_SoftFactor", InputDataDesc::ID_FLOAT, OFFSET(EVA2_SoftFactor),
     "0.4"}, //Turnradius multiplier to get safe distance from ground for OBSTACLE in SLOW
    {"EVA1_MedFactor", InputDataDesc::ID_FLOAT, OFFSET(EVA1_MedFactor),
     "0.8"}, //Turnradius multiplier to get safe distance from ground for FLY_UP in MED
    {"EVA2_MedFactor", InputDataDesc::ID_FLOAT, OFFSET(EVA2_MedFactor),
     "0.6"}, //Turnradius multiplier to get safe distance from ground for OBSTACLE in MED
    {"EVA1_HardFactor", InputDataDesc::ID_FLOAT, OFFSET(EVA1_HardFactor),
     "1.2"}, //Turnradius multiplier to get safe distance from ground for FLY_UP in HARD
    {"EVA2_HardFactor", InputDataDesc::ID_FLOAT, OFFSET(EVA2_HardFactor),
     "0.9"}, //Turnradius multiplier to get safe distance from ground for FLY_UP in MED
    {"TFR_GammaCorrMult", InputDataDesc::ID_FLOAT, OFFSET(TFR_GammaCorrMult),
     "1.0"}, //Turnradius multiplier to get safe distance from ground for OBSTACLE in HARD
    {"LantirnCameraX", InputDataDesc::ID_FLOAT, OFFSET(LantirnCameraX),
     "10.0"}, //Position of the camera
    ...
    {"MinTGTMAR", InputDataDesc::ID_FLOAT, OFFSET(minTGTMAR),
     "0.0"}, // 2002-03-22 ADDED BY S.G. Min TGTMAR for this type of aicraft
    {"MaxMARIdedStart", InputDataDesc::ID_FLOAT, OFFSET(maxMARIdedStart),
     "0.0"}, // 2002-03-22 ADDED BY S.G. MinMAR for this type of aicraft when target is ID'ed and below 5K
    {"AddMARIded5k", InputDataDesc::ID_FLOAT, OFFSET(addMARIded5k),
     "0.0"}, // 2002-03-22 ADDED BY S.G. MinMAR for this type of aicraft when target is ID'ed and below 5K
    {"AddMARIded18k", InputDataDesc::ID_FLOAT, OFFSET(addMARIded18k),
     "0.0"}, // 2002-03-22 ADDED BY S.G. MinMAR for this type of aicraft when target is ID'ed and below 18K
    {"AddMARIded28k", InputDataDesc::ID_FLOAT, OFFSET(addMARIded28k),
     "0.0"}, // 2002-03-22 ADDED BY S.G. MinMAR for this type of aicraft when target is ID'ed and below 28K
    {"AddMARIdedSpike", InputDataDesc::ID_FLOAT, OFFSET(addMARIdedSpike),
     "0.0"}, // 2002-03-22 ADDED BY S.G. MinMAR for this type of aicraft when target is ID'ed and spiked

    // 2003-09-30 added by MLR to support Aircraft animations
    {"animEngineRPMMult", InputDataDesc::ID_FLOAT, OFFSET(animEngineRPMMult),
     "1000.0"},

    {"animSpoiler1Max", InputDataDesc::ID_FLOAT, OFFSET(animSpoiler1Max),
     "0.0"},
    {"animSpoiler1Rate", InputDataDesc::ID_FLOAT, OFFSET(animSpoiler1Rate),
     "45.0"},
    {"animSpoiler1OffAtWingSweep", InputDataDesc::ID_FLOAT,
     OFFSET(animSpoiler1OffAtWingSweep), "70.0"},
    {"animSpoiler1AirBrake", InputDataDesc::ID_INT,
     OFFSET(animSpoiler1AirBrake), "1"},
    {"animSpoiler2Max", InputDataDesc::ID_FLOAT, OFFSET(animSpoiler2Max),
     "0.0"},
    {"animSpoiler2Rate", InputDataDesc::ID_FLOAT, OFFSET(animSpoiler2Rate),
     "20.0"},
    {"animSpoiler2OffAtWingSweep", InputDataDesc::ID_FLOAT,
     OFFSET(animSpoiler2OffAtWingSweep), "45.0"},
    {"animSpoiler2AirBrake", InputDataDesc::ID_INT,
     OFFSET(animSpoiler2AirBrake), "1"},
    ...
    {"animStrobeOnTime", InputDataDesc::ID_FLOAT, OFFSET(animStrobeOnTime),
     "0.08"},
    {"animStrobeOffTime", InputDataDesc::ID_FLOAT, OFFSET(animStrobeOffTime),
     "2.0"},

    {"animWingFlashOnTime", InputDataDesc::ID_FLOAT,
     OFFSET(animWingFlashOnTime),
     "0.4"}, // New variable for dat files - MartinV
    {"animWingFlashOffTime", InputDataDesc::ID_FLOAT,
     OFFSET(animWingFlashOffTime),
     "0.5"}, // New variable for dat files - MartinV
    ...
    {"animThrRevAngle", InputDataDesc::ID_FLOAT, OFFSET(animThrRevAngle),
     "60.0"}, // Thrust Reversers - FRB
    {"animThrRevRate", InputDataDesc::ID_FLOAT, OFFSET(animThrRevRate),
     "10.0"}, // Thrust Reversers - FRB

    // RV - Biker - Catapult thrust multiplier
    {"catapultThrustMultiplier", InputDataDesc::ID_FLOAT,
     OFFSET(catapultThrustMultiplier), "4.0"},
    ...
    {"hasProgrammableEWS", InputDataDesc::ID_INT, OFFSET(hasProgrammableEWS),
     "1"},
    ...
    {"animAileronRate", InputDataDesc::ID_FLOAT, OFFSET(animAileronRate),
     "45.0"},

    {"animSwingWingStages", InputDataDesc::ID_INT, OFFSET(animSwingWingStages),
     "2"},
    {"animSwingWingMach1", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingMach[0]), "0.6"},
    {"animSwingWingMach2", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingMach[1]), "0.8"},
    {"animSwingWingMach3", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingMach[2]), "0.0"},
    {"animSwingWingMach4", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingMach[3]), "0.0"},
    {"animSwingWingMach5", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingMach[4]), "0.0"},
    {"animSwingWingMach6", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingMach[5]), "0.0"},
    {"animSwingWingMach7", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingMach[6]), "0.0"},
    {"animSwingWingMach8", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingMach[7]), "0.0"},
    {"animSwingWingMach9", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingMach[8]), "0.0"},
    {"animSwingWingMach10", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingMach[9]), "0.0"},
    {"animSwingWingAngle1", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingAngle[0]), "25.0"},
    {"animSwingWingAngle2", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingAngle[1]), "50.0"},
    {"animSwingWingAngle3", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingAngle[2]), "0.0"},
    {"animSwingWingAngle4", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingAngle[3]), "0.0"},
    {"animSwingWingAngle5", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingAngle[4]), "0.0"},
    {"animSwingWingAngle6", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingAngle[5]), "0.0"},
    {"animSwingWingAngle7", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingAngle[6]), "0.0"},
    {"animSwingWingAngle8", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingAngle[7]), "0.0"},
    {"animSwingWingAngle9", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingAngle[8]), "0.0"},
    {"animSwingWingAngle10", InputDataDesc::ID_FLOAT,
     OFFSET(animSwingWingAngle[9]), "0.0"},
    {"animSwingWingRate", InputDataDesc::ID_FLOAT, OFFSET(animSwingWingRate),
     "5.0"},

    {"animWheelRadius1", InputDataDesc::ID_FLOAT, OFFSET(animWheelRadius[0]),
     "0.0"},
    {"animWheelRadius2", InputDataDesc::ID_FLOAT, OFFSET(animWheelRadius[1]),
     "0.0"},
    {"animWheelRadius3", InputDataDesc::ID_FLOAT, OFFSET(animWheelRadius[2]),
     "0.0"},
    {"animWheelRadius4", InputDataDesc::ID_FLOAT, OFFSET(animWheelRadius[3]),
     "0.0"},
    {"animWheelRadius5", InputDataDesc::ID_FLOAT, OFFSET(animWheelRadius[4]),
     "0.0"},
    {"animWheelRadius6", InputDataDesc::ID_FLOAT, OFFSET(animWheelRadius[5]),
     "0.0"},
    {"animWheelRadius7", InputDataDesc::ID_FLOAT, OFFSET(animWheelRadius[6]),
     "0.0"},
    {"animWheelRadius8", InputDataDesc::ID_FLOAT, OFFSET(animWheelRadius[7]),
     "0.0"},

    {"animRefuelAngle", InputDataDesc::ID_FLOAT, OFFSET(animRefuelAngle),
     "1.0"},
    {"animRefuelRate", InputDataDesc::ID_FLOAT, OFFSET(animRefuelRate), ".5"},

    {"fuelGaugeMultiplier", InputDataDesc::ID_FLOAT,
     OFFSET(fuelGaugeMultiplier), "10"},

    {"animGearMaxComp1", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxComp[0]),
     "0.0"},
    {"animGearMaxComp2", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxComp[1]),
     "0.0"},
    {"animGearMaxComp3", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxComp[2]),
     "0.0"},
    {"animGearMaxComp4", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxComp[3]),
     "0.0"},
    {"animGearMaxComp5", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxComp[4]),
     "0.0"},
    {"animGearMaxComp6", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxComp[5]),
     "0.0"},
    {"animGearMaxComp7", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxComp[6]),
     "0.0"},
    {"animGearMaxComp8", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxComp[7]),
     "0.0"},

    {"animGearMaxExt1", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxExt[0]),
     "0.0"},
    {"animGearMaxExt2", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxExt[1]),
     "0.0"},
    {"animGearMaxExt3", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxExt[2]),
     "0.0"},
    {"animGearMaxExt4", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxExt[3]),
     "0.0"},
    {"animGearMaxExt5", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxExt[4]),
     "0.0"},
    {"animGearMaxExt6", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxExt[5]),
     "0.0"},
    {"animGearMaxExt7", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxExt[6]),
     "0.0"},
    {"animGearMaxExt8", InputDataDesc::ID_FLOAT, OFFSET(animGearMaxExt[7]),
     "0.0"},

    {"sndExternalVol", InputDataDesc::ID_FLOAT, OFFSET(sndExternalVol),
     "-2000.0"},
    {"sndInt", InputDataDesc::ID_INT, OFFSET(sndInt),
     "-1"}, // MLR 2003-11-10 internal engine sound
    // MLR 1/8/2004 -
    {"sndIntChart", InputDataDesc::ID_LOOKUPTABLE, OFFSET(sndIntChart),
     "1  0 1"},
    {"sndAbIntChart", InputDataDesc::ID_LOOKUPTABLE, OFFSET(sndAbIntChart),
     "2  0 0  1 1"},
    {"sndInt2Chart", InputDataDesc::ID_LOOKUPTABLE, OFFSET(sndInt2Chart),
     "3  0 0  1 1  1.035 1"},
    {"sndExtChart", InputDataDesc::ID_LOOKUPTABLE, OFFSET(sndExtChart),
     "1  0 1"},
    {"sndAbExtChart", InputDataDesc::ID_LOOKUPTABLE, OFFSET(sndAbExtChart),
     "2  0 0  1 1"},
    {"sndExt2Chart", InputDataDesc::ID_LOOKUPTABLE, OFFSET(sndExt2Chart),
     "3  0 0  1 1  1.035 1"},

    {"sndIntPitchChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndIntPitchChart), "2 0 0 1.03 1.47"},
    {"sndAbIntPitchChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndAbIntPitchChart), "2 0 0 1.03 1.47"},
    {"sndInt2PitchChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndInt2PitchChart), "2 0 0 1.03 1.47"},
    {"sndExtPitchChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndExtPitchChart), "2 0 0 1.03 1.47"},
    {"sndAbExtPitchChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndAbExtPitchChart), "2 0 0 1.03 1.47"},
    {"sndExt2PitchChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndExt2PitchChart), "2 0 0 1.03 1.47"},
    ...
    {"sndAero1AOAChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndAeroAOAChart[0]), "1 0 0"},
    {"sndAero2AOAChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndAeroAOAChart[1]), "1 0 0"},
    {"sndAero3AOAChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndAeroAOAChart[2]), "1 0 0"},
    {"sndAero4AOAChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndAeroAOAChart[3]), "1 0 0"},

    {"sndAero1SpeedChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndAeroSpeedChart[0]), "1 0 0"},
    {"sndAero2SpeedChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndAeroSpeedChart[1]), "1 0 0"},
    {"sndAero3SpeedChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndAeroSpeedChart[2]), "1 0 0"},
    {"sndAero4SpeedChart", InputDataDesc::ID_LOOKUPTABLE,
     OFFSET(sndAeroSpeedChart[3]), "1 0 0"},
    ...
    {"typeAC", InputDataDesc::ID_INT, OFFSET(typeAC),
     "0"}, //TJL 02/08/04 Used to set aircraft specific features
    {"typeEngine", InputDataDesc::ID_INT, OFFSET(typeEngine),
     "0"}, //TJL 02/08/04 Used to set engine specific features

    {"elevRate", InputDataDesc::ID_FLOAT, OFFSET(elevRate),
     "90"}, // MLR 1/14/2004 - Stab rates
    {"gunPitch", InputDataDesc::ID_FLOAT, OFFSET(gunElevation),
     "0"}, // MLR 1/28/2004 -
    {"gunYaw", InputDataDesc::ID_FLOAT, OFFSET(gunAzimuth),
     "0"}, // MLR 1/28/2004 -

    {"MeanTimeBetweenFailures", InputDataDesc::ID_FLOAT,
     OFFSET(MeanTimeBetweenFailures), "0"}, //Wombat778 2-24-04
    ...
    {"A2GClusterAlt", InputDataDesc::ID_FLOAT, OFFSET(A2GClusterAlt),
     "15000.0"},
    {"A2GDumbLDAlt", InputDataDesc::ID_FLOAT, OFFSET(A2GDumbLDAlt), "15000.0"},
    {"A2GGenericBombAlt", InputDataDesc::ID_FLOAT, OFFSET(A2GGenericBombAlt),
     "1000.0"},
    {"A2GGunRocketAlt", InputDataDesc::ID_FLOAT, OFFSET(A2GGunRocketAlt),
     "8000.0"},
    {"A2GCameraAlt", InputDataDesc::ID_FLOAT, OFFSET(A2GCameraAlt), "12000.0"},
    {"A2GBombMissileAlt", InputDataDesc::ID_FLOAT, OFFSET(A2GBombMissileAlt),
     "13000.0"},

    {"swingwingHinge", InputDataDesc::ID_VECTOR, OFFSET(swingWingHinge),
     "2 3 0"},
    //Cobra 10/30/04 TJL
    {"animIntakeRamp1", InputDataDesc::ID_2DTABLE, OFFSET(animIntakeRamp[0]),
     "0 0"},
    {"animIntakeRamp2", InputDataDesc::ID_2DTABLE, OFFSET(animIntakeRamp[1]),
     "0 0"},
    {"animIntakeRamp3", InputDataDesc::ID_2DTABLE, OFFSET(animIntakeRamp[2]),
     "0 0"},

    {"FTITStart", InputDataDesc::ID_FLOAT, OFFSET(FTITStart),
     "5.0"}, //TJL 09/11/04
    {"FTITIdle", InputDataDesc::ID_FLOAT, OFFSET(FTITIdle),
     "7.0"}, //TJL 09/11/04
    {"FTITMax", InputDataDesc::ID_FLOAT, OFFSET(FTITMax), "8.9"}, //TJL 09/11/04
    {"criticalAOA", InputDataDesc::ID_FLOAT, OFFSET(criticalAOA),
     "0.0"}, //TJL 09/11/04
    {"hasIFF", InputDataDesc::ID_INT, OFFSET(hasIFF), "0"}, //Cobra 11/20/04
    {"hasThrRev", InputDataDesc::ID_INT, OFFSET(hasThrRev),
     "0"}, //Cobra 02/06/05
    {NULL}};
#undef OFFSET
...
EngineData *AirframeEngineRead(SimlibFileClass *inputFile)
{
...
#ifdef USE_SH_POOLS
    engineData->mach =
        (float *)MemAllocPtr(gReadInMemPool, sizeof(float) * numMach, 0);
#else
    ...
    /*----------------------------------------------*/
#ifdef USE_SH_POOLS
    engineData->thrust[0] = (float *)MemAllocPtr(
        gReadInMemPool, sizeof(float) * numMach * numAlt, 0);
    engineData->thrust[1] = (float *)MemAllocPtr(
        gReadInMemPool, sizeof(float) * numMach * numAlt, 0);
    engineData->thrust[2] = (float *)MemAllocPtr(
        gReadInMemPool, sizeof(float) * numMach * numAlt, 0);
#else
...
#ifdef USE_SH_POOLS
        engineData->fuelflow[0] = (float *)MemAllocPtr(
            gReadInMemPool, sizeof(float) * numMach * numAlt, 0);
        engineData->fuelflow[1] = (float *)MemAllocPtr(
            gReadInMemPool, sizeof(float) * numMach * numAlt, 0);
        engineData->fuelflow[2] = (float *)MemAllocPtr(
            gReadInMemPool, sizeof(float) * numMach * numAlt, 0);
#else
    ...
    for (alt = 0; alt < numAlt and not engineData->hasAB; alt++)
        for (mach = 0; mach < numMach and not engineData->hasAB; mach++)
            if (engineData->thrust[1][alt * numMach + mach] not_eq
                engineData->thrust[2][alt * numMach + mach])
...
RollData *AirframeFcsRead(SimlibFileClass *inputFile)
{
...
#ifdef USE_SH_POOLS
    rollCmd->alpha =
        (float *)MemAllocPtr(gReadInMemPool, sizeof(float) * numAlpha, 0);
#else
...
#ifdef USE_SH_POOLS
    rollCmd->qbar =
        (float *)MemAllocPtr(gReadInMemPool, sizeof(float) * numQbar, 0);
#else
    ...
    /*-------------------------------------*/
#ifdef USE_SH_POOLS
    rollCmd->roll = (float *)MemAllocPtr(gReadInMemPool,
                                         sizeof(float) * numQbar * numAlpha, 0);
#else
    ...
    for (alpha = 0; alpha < numAlpha; alpha++)
    {
        for (qbar = 0; qbar < numQbar; qbar++)
        {
            rollCmd->roll[alpha * numQbar + qbar] =
                (float)atof(inputFile->GetNext()) * scale;
        }
    }
