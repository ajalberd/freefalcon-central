...
struct
    D3D12Texture; // #DX12 п.5 A1: in-scene VR menu RTT (from D3D12TextureManager)

class D3D12Backend
    : public IRenderBackend // Artscout - 2026: #DX12 -- implements the neutral backend surface
{
public:
    D3D12Backend();
    ~D3D12Backend();
    ...
    void BindBackBuffer(
        bool
            bClearDepth); // Phase 1: same as BeginFrame(clear) -- one RT, no depth yet
    ...
    void BlitBitmap565(const void* pSrc565, int srcW,
                       int srcH); // opaque fullscreen blit (menu frame)
    ...
    {
        return m_frameIndex;
    } // which of the kFrameCount ring slots is live
    ...
    void ReadbackRttTo565(void* rttHandle, unsigned short* dst,
                          int dstStridePix, int dstHeightPix, int x, int y,
                          int w, int h);
    ...
    void BeginStereoInstancedFrame(void* arrayImg, unsigned __int64 arrayRtvPtr,
    ...
    unsigned RenderEpoch() const
    {
        return m_renderEpoch;
    }
    ...
    // Artscout - 2026 (#104): now an IRenderBackend override, so the engine can ask the ACTIVE backend for its scene
    // size instead of naming D3D12. Behaviour unchanged.
    int SceneW() const override
    {
        return m_sceneW > 0 ? m_sceneW : m_nWidth;
    }
    int SceneH() const override
    {
        return m_sceneH > 0 ? m_sceneH : m_nHeight;
    }
    ...
    void EnsureMenuRtt(
        int w,
        int h); // (re)create at this size, best-effort (cached; recreated on size change)
    void BindMenuRtt(
        bool
            clear); // bind the menu color RTV + its depth, set viewport, optionally clear
    void*
    MenuRttTex(); // D3D12Texture* (copy source for the XR UI swapchain); NULL if not up
    ...
    static int
    DepthFormat(); // DXGI_FORMAT_D32_FLOAT_S8X24_UINT (reversed-Z float depth + stencil)
    ...
    {
        return m_curSampleCount > 0 ? m_curSampleCount : 1;
    }
    bool MsaaActive() const
    {
...
private:
    ...
    struct ID3D12Resource*
        m_depthSrvFor; // #13: which resource m_pDepthSrvHeap's descriptor describes
    ...
    unsigned __int64
    SignalQueue(); // post the next strictly-increasing value on m_pQueue
    void WaitForFence(unsigned __int64 v); // block until the fence reaches v
    void
    BeginCommandList(); // wait out m_pAlloc[m_frameIndex], then reset alloc + list
    unsigned __int64 NextSignalValue() const
    {
        return m_fenceCounter + 1;
    } // value the NEXT SignalQueue() posts
    ...
    ID3D12Resource*
        m_pEyeDepthTex; // #DX12 п.5 -- VR per-eye depth (sized to the eye)
    ...
    struct ViColorSlot
    {
        ...
        unsigned __int64 arrayRtv;
        unsigned __int64 sliceRtv[2];
        int w, h;
        int fmt;
        unsigned lru;
    };
    ...
    void*
        m_pEyeResolveImg; // XR eye image to resolve MSAA into (saved in BeginEyeFrame)
    int ResolveMsaaSamples(); // read cfg + snap to a supported level (1 = off)
    bool
    CreateMsaaTargets(int w,
                      int h); // (re)create the MSAA color+depth at this size
    ...
    ID3D12Resource*
        m_pMenuDepthTex; // its depth (exit dialog is a 3D BSP -> needs Z)
    ...
    unsigned __int64
        m_curRtvPtr; // #DX12: currently bound RTV (backbuffer OR the RTT atlas) -- ClearCurrentRTV clears THIS
    ...
    unsigned __int64 m_allocFence
        [kFrameCount]; // value signalled after m_pAlloc[i]'s work was submitted
    unsigned __int64
        m_fenceCounter; // last value posted on m_pQueue (strictly increasing)
    HANDLE m_fenceEvent;
    ...
    ID3D12PipelineState*
        m_pQuadPSOBlend; // #DX12 п.2: alpha-blend variant (UI-over-3D composite)
    ID3D12DescriptorHeap*
        m_pSrvHeap; // shader-visible CBV/SRV heap (1 SRV = the UI texture)
    ID3D12Resource*
        m_pQuadTex; // DEFAULT-heap RGBA8 texture (sampled by the quad)
    ID3D12Resource*
        m_pQuadUpload; // UPLOAD-heap staging buffer (565->RGBA8 each frame)
