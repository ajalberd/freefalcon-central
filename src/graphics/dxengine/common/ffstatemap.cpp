...
//-----------------------------------------------------------------------------
// Artscout - 2026 (#104): no stdafx.h -- this translation is pure (a switch over STATE_* filling a POD) and has to
// stay buildable outside the Windows engine, since the Linux ffvulkan lib compiles it too.
#include "ffstatemap.h" // lowercase: the real filename. Windows does not care, a case-sensitive Linux FS does.

// Small helper to fill a descriptor compactly.
static FFStateDesc Make(unsigned int flags, FFBlendMode blend = BLEND_OPAQUE,
                        FFFilterMode filter = FILTER_LINEAR,
                        FFAddrMode addr = ADDR_WRAP, bool depthWrite = true,
                        bool depthTest = true)
{
    FFStateDesc d;
    d.flags = flags;
...
bool FFMapState(int s, FFStateDesc& out)
{
    ...
    case STATE_SOLID:
        ...
        return true;
    case STATE_LIT:
        out = Make(FF_VERTEXCOLOR | FF_LIGHTING);
        return true;
    case STATE_GOURAUD:
        out = Make(FF_VERTEXCOLOR | FF_LIGHTING);
        return true;
    ...
    case STATE_LANDSCAPE_GOURAUD:
        out = Make(FF_TEXTURE0 | FF_VERTEXCOLOR | FF_FOG);
        return true;
    ...
    case STATE_CHROMA_TEXTURE_PERSPECTIVE:
        out = Make(FF_TEXTURE0 | FF_CHROMAKEY);
        return true;
    ...
    case STATE_CHROMA_TEXTURE_GOURAUD2: // 3D-pit HUD/MFD RTT (D3D7 MPR_SE_CHROMA2)
        ...
        out = Make(FF_TEXTURE0 | FF_CHROMAKEY | FF_VERTEXCOLOR, BLEND_OPAQUE,
    ...
    case STATE_ALPHA_TEXTURE_PERSPECTIVE:
        out = Make(FF_TEXTURE0, BLEND_ALPHA, FILTER_LINEAR, ADDR_WRAP, false,
                   /*depthTest*/ false);
        return true;
    ...
    case STATE_ALPHA_TEXTURE_GOURAUD_PERSPECTIVE:
        out = Make(FF_TEXTURE0 | FF_VERTEXCOLOR, BLEND_ALPHA, FILTER_LINEAR,
                   ADDR_WRAP, false, /*depthTest*/ false);
        return true;
    ...
    case STATE_ALPHA_TEXTURE_NOFILTER_PERSPECTIVE:
        out = Make(FF_TEXTURE0, BLEND_ALPHA, FILTER_POINT, ADDR_WRAP, false,
                   /*depthTest*/ false);
        return true;
    case STATE_ALPHA_TEXTURE_PERSPECTIVE_CLAMP:
        out = Make(FF_TEXTURE0, BLEND_ALPHA, FILTER_LINEAR, ADDR_CLAMP, false,
                   /*depthTest*/ false);
        return true;

    // ---- multitexture ----
    case STATE_MULTITEXTURE:
        out = Make(FF_TEXTURE0 | FF_TEXTURE1 | FF_FOG);
        return true;

    // Artscout - 2026: #12: water terrain tiles -- same as multitexture terrain plus the animated water effect.
    case STATE_WATER:
        out = Make(FF_TEXTURE0 | FF_TEXTURE1 | FF_FOG | FF_WATER);
        return true;
    case STATE_MULTITEXTURE_ALPHA:
        out = Make(FF_TEXTURE0 | FF_TEXTURE1 | FF_FOG, BLEND_ALPHA,
                   FILTER_LINEAR, ADDR_WRAP, false, /*depthTest*/ false);
        return true;
