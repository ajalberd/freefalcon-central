...
extern float
    g_fSunWarmth; // #96 sun disc warmth (0 = physical white, 1 = strongly yellow)
extern float g_fSkyDomeRadius, g_fSkyDomeSunSize, g_fSkyDomeMoonSize,
    g_fSkyDomeStarSize;
extern float
    g_fSkyMapRotate; // #96: equirect starmap azimuth rotation (0..1 = full turn) to place the Milky Way
extern float
    g_fSkyMapBright; // #96: starmap brightness multiplier (additive over the night sky)
extern float
    g_fSkyMapTilt; // #96: starmap tilt OFFSET (turns*PI) on top of the auto latitude tilt
extern float
    g_fLatitude; // theater latitude (deg) -- drives the auto celestial-pole tilt (Polaris to the N)
...
const float RenderOTW::HAZE_ALTITUDE_FACTOR =
    (1.0f / (SKY_MAX_HEIGHT - SKY_ROOF_HEIGHT));
...
static void ShiftHorizonOffAxis(HorizonRecord* h, float oaX, float oaY,
                                float sR, float cR)
{
    const float perp =
        oaX * sR + oaY * cR; // component perpendicular to the horizon line
    if (perp == 0.0f)
        return;
...
void RenderOTW::DrawSkyBelow(void)
{
    ...
    if (drawClear)
    {
        ...
        u = (float)fmod(
            viewpoint->Y() * 0.5f * ROOF_REPEAT_COUNT / SKY_ROOF_RANGE, 1.0f);
        v = 1.0f - (float)fmod(viewpoint->X() * 0.5f * ROOF_REPEAT_COUNT /
                                   SKY_ROOF_RANGE,
                               1.0f);

        // South West
        worldSpace.x = viewpoint->X() - SKY_ROOF_RANGE,
        worldSpace.y = viewpoint->Y() - SKY_ROOF_RANGE;
        ...
        v3.u = u + ROOF_REPEAT_COUNT, v3.v = v + ROOF_REPEAT_COUNT,
        v3.q = v3.csZ * Q_SCALE;
...
void RenderOTW::DrawSkyAbove(void)
{
    ...
    float bandAngleUp =
        min(PI / 18.0f, (PI / 48.0f) * (SKY_MAX_HEIGHT / vpAlt));
    ...
    if (drawClouds)
    {
        ...
        worldSpace.y = viewpoint->Y() + SKY_ROOF_RANGE;
        TransformPoint(&worldSpace, &v2);
        v2.u = u + ROOF_REPEAT_COUNT, v2.v = v + ROOF_REPEAT_COUNT,
        v2.q = v2.csZ * Q_SCALE;
...
void RenderOTW::DrawClearSky(HorizonRecord* pHorizon)
{
    ...
    horizonLine.SetupWithVector(shiftX + pHorizon->vxUp,
                                shiftY + pHorizon->vyUp, pHorizon->hx,
                                pHorizon->hy);
    ...
    if (num >= 3)
    {
        ...
        context.SelectForegroundColor(
            ((FloatToInt32(sky_color.r * 255.9f)) +
             ...
             (FloatToInt32(sky_color.b * 255.9f) << 16)) +
            0xff000000);

        context.DrawPrimitive(MPR_PRM_TRIFAN, 0, num, &vert[0],
                              sizeof(vert[0]));
    }
}
...
void RenderOTW::DrawSkyHazeBand(struct HorizonRecord* pHorizon)
{
    ...
    if (pHorizon->horeffect)
    {
        ...
        v0.g = /*sunLeft ? 0.0f : 1.0f;*/ haze_sky_color.g +
               dg * pHorizon->rhazescale;
        v0.b = haze_sky_color.b + db * pHorizon->rhazescale;

        v1.r = haze_sky_color.r + dr * pHorizon->lhazescale;
        v1.g = /*sunLeft ? 1.0f : 0.0f;*/ haze_sky_color.g +
               dg * pHorizon->lhazescale;
    ...
    if (starblend > 0.000001f)
    {
        ...
        for (i = 0; i < stardata->totalcoord; i++, coord++)
        {
            ...
            if (lastcolor not_eq coord->color)
            {
                ...
                if (star_color.r > 255.0f)
                    star_color.r = 255.0f;

                if (star_color.g > 255.0f)
                    star_color.g = 255.0f;

                if (star_color.b > 255.0f)
                    star_color.b = 255.0f;

                if (star_color.r < 64.0f)
                    star_color.r = 64.0f;

                if (star_color.g < 64.0f)
                    star_color.g = 64.0f;

                if (star_color.b < 64.0f)
                    star_color.b = 64.0f;

                ProcessColor(&star_color);
                draw_color = (DWORD)star_color.r | (DWORD)star_color.g << 8 |
            ...
            if (scratch_z < 0.000001f)
                continue;

            if (GetHorizontalClipFlags(scratch_x, scratch_z) not_eq ON_SCREEN)
                continue;

            if (GetVerticalClipFlags(scratch_y, scratch_z) not_eq ON_SCREEN)
                continue;
...
{
    if (g_bVrDeferCelestial)
        return; // suppressed in the VI world pass; re-drawn per eye in the tail
    Tpoint center;
    float alpha;
    float dist;
    ...
    if (DisplayOptions.m_texMode == DisplayOptionsClass::TEX_MODE_DDS)
    {
        ...
        DrawCelestialBody(&center, dist, 1.f, 0.984375f, 0.9765625f,
                          0.87109375f);
    ...
    else
    {
        ...
        context
            .FlushPending(); // #79: submit while SunTexture is bound (see above)
    }
...
void RenderOTW::DrawMoon(void)
{
    ...
    context
        .FlushPending(); // Artscout - 2026 (#79): submit while MoonTexture is bound to slot 0 (see DrawSun)
}
...
void RenderOTW::VrDrawCelestial(void)
{
    ...
    bool save = g_bVrDeferCelestial;
    g_bVrDeferCelestial = false;
    if (TheTimeOfDay.ThereIsASun())
        DrawSun();
    if (TheTimeOfDay.ThereIsAMoon())
        DrawMoon();
...
struct SkyDomeVert
{
    ...
    unsigned int spec;
    float tu, tv;
}; // col/spec MUST be 32-bit (int, not long): long=8B on LP64 shifts tu/tv & stride off the pipeline layout -> broken sky on Linux
...
static inline unsigned long PackSkyColor(float r, float g, float b, float a)
{
    ...
    if (a > 1)
        a = 1;
    unsigned R = (unsigned)(r * 255.0f), G = (unsigned)(g * 255.0f),
             B = (unsigned)(b * 255.0f), A = (unsigned)(a * 255.0f);
    return (A << 24) | (R << 16) | (G << 8) |
           B; // 0xAARRGGBB, matches TGpuVert / the object vertex colour packing
}
...
static void* SkyGlareTex()
{
    ...
    if (!s_tried && g_pRenderer)
    {
        s_tried = true;
        const int gd = 64;
        unsigned* gg = new unsigned[(size_t)gd * gd];
        ...
        for (int y = 0; y < gd; ++y)
            for (int x = 0; x < gd; ++x)
            {
                ...
                const unsigned A =
                    (unsigned)(255.0f * a +
                               0.5f); // kept sane in case it is ever alpha-blended
                gg[y * gd + x] =
                    (A << 24) | (C << 16) | (C << 8) |
                    C; // R8G8B8A8: white premultiplied by the falloff
            }
...
void RenderOTW::DrawSkyBillboard(const void* dirv, float R, float sz, float r,
                                 float g, float b, void* srv, float a,
                                 float flatten, bool additive)
{
    const float szY =
        sz *
        flatten; // refraction squashes the disc vertically near the horizon (never magnifies)
    ...
    right.z = 0.0f;
    float rl =
        (float)sqrt(right.x * right.x + right.y * right.y + right.z * right.z);
    if (rl < 1e-6f)
    {
        right.x = 1;
        right.y = 0;
    ...
    const float cx0 = dir->x * R, cy0 = dir->y * R,
                cz0 = dir->z * R; // disc centre (camera-relative)
    ...
    if (srv)
    {
        SkyDomeVert q[4];
        unsigned short qi[6];
        const unsigned long qc = PackSkyColor(
            r, g, b, a); // a<1 -> the disc washes out (pale daytime moon)
        for (int k = 0; k < 4; ++k)
        {
        ...
        qi[4] = 3;
        qi[5] = 2;
        g_pRenderer->BeginSkyPass(
            true); // alpha blend (texture alpha = round shape)
        ...
        g_pRenderer->SetTexture(
            0,
            0); // clear slot 0 so a later dome/draw doesn't inherit the sun/moon texture
        return;
    }
    ...
    {
        di[n++] = 0;
        di[n++] = (unsigned short)(1 + k);
        di[n++] = (unsigned short)(1 + ((k + 1) % NS));
    }

    g_pRenderer->BeginSkyPass(
        false); // OPAQUE (no alpha premultiply-to-black); drawn over the dome, depth off
    g_pRenderer->SetProj((const float*)&CDXEngine::GetObjProjection());
    g_pRenderer->SetView((const float*)&CDXEngine::GetObjView());
    g_pRenderer->SetTexture(0,
                            0); // textureless disc; sun/moon texture = phase 2
...
void RenderOTW::DrawSkyDome(void)
{
    ...
    if (sunElev < 0.0f)
        sunElev = 0.0f;
    if (sunElev > 1.0f)
        sunElev = 1.0f; // clamped, moon gate
    ...
    float glowWin = 1.0f - (float)fabs(sunUp) / 0.30f;
    if (glowWin < 0.0f)
        glowWin = 0.0f;
    ...
    const float SUN_GLARE_HALF =
        0.10f; // ~11 deg across -- the sun's glare is what you cannot look at
    const float MOON_GLARE_HALF =
        0.045f; // ~5 deg across -- keeps the pre-shrink halo the moon read well with
    ...
    const float elLo =
        -0.12f; // ~ -7 deg (skirt below the horizon so the dome overlaps the terrain edge)
    const float elHi =
        (float)(PI *
                0.5); // 90 deg -- CLOSE the dome at the zenith. Capping short of the pole left a
    ...
    for (int ri = 0; ri <= RINGS; ++ri)
    {
        ...
        float gh = el * (1.0f / GROUND_HAZE_BLEND);
        if (gh < 0.0f)
            gh = 0.0f;
        if (gh > 1.0f)
            gh = 1.0f;
        float gf = el * (1.0f / GLOW_HORIZON_FADE);
        if (gf < 0.0f)
            gf = 0.0f;
        if (gf > 1.0f)
            gf = 1.0f; // glow ramp
        const float hr =
            fogc->r + (haze_sky_color.r - fogc->r) *
                          gh; // el<=0 -> exactly the terrain's fog colour
        ...
        for (int si = 0; si <= SEGS; ++si)
        {
            ...
            const float dx = ce * ca, dy = ce * sa,
                        dz = -se; // world dir (x=N, y=E, z=down -> up=-z)
            SkyDomeVert& o = v[nv++];
            o.p[0] = dx * R;
            o.p[1] = dy * R;
            o.p[2] = dz * R;
            ...
            const float glow = sdot * sdot * sdot * (1.0f - tc) * gf * glowWin *
                               0.6f; // glowWin: only near dawn/dusk
            ...
            // (geometrically correct, no vertical stretch; visible hemisphere = the map half the pole faces).
            {
                ...
                const float tl =
                    ((90.0f - g_fLatitude) * (1.0f / 180.0f) + g_fSkyMapTilt) *
                    (float)PI;
                const float ct = (float)cos(tl), st = (float)sin(tl);
                const float tx =
                    dx * ct +
                    dz * st; // rotate (dx=north, dz=down) about East (+y)
                const float tz = -dx * st + dz * ct;
                const float ty = dy;
                const float rawU =
                    0.5f + (float)atan2(ty, tx) / (float)(2.0 * PI);
                if (si > 0)
                {
                    const float du =
                        rawU - prevRawU; // unwrap across the atan2 branch cut
                    if (du < -0.5f)
                        ringOfs += 1.0f;
                    else if (du > 0.5f)
                        ringOfs -= 1.0f;
                ...
                if (up2 < -1.0f)
                    up2 = -1.0f;
                float sv = 0.5f - (float)asin(up2) / (float)PI;
                if (sv < 0.0f)
                    sv = 0.0f;
                if (sv > 1.0f)
                    sv = 1.0f;
    ...
    {
        s_starmapTried = true;
        if (void* sm = (void*)g_pRenderer->LoadTextureFile(
                "terrdata/misctex/starmap.png"))
            s_starmapSrv = sm;
        else if (void* sj = (void*)g_pRenderer->LoadTextureFile(
                     "terrdata/misctex/starmap.jpg"))
            s_starmapSrv = sj;
        else if (void* sk = (void*)g_pRenderer->LoadTextureFile(
                     "terrdata/misctex/starmap.dds"))
            s_starmapSrv = sk;
    }
    if (s_starmapSrv and sb > 0.003f)
    {
        ...
        const float mb =
            (g_fSkyMapBright > 0.0f ? g_fSkyMapBright : 0.85f) * sb;
        ...
        for (int k = 0; k < nv; ++k)
        {
            ...
            const float el2 =
                elLo + (elHi - elLo) * ((float)ri2 / (float)RINGS);
            float sf = el2 * (1.0f / STAR_HORIZON_FADE);
            if (sf < 0.0f)
                sf = 0.0f;
            if (sf > 1.0f)
                sf = 1.0f;
        ...
        g_pRenderer->SetCameraPos(cam.x, cam.y, cam.z);
        g_pRenderer->SetTexture(0,
                                (struct ID3D11ShaderResourceView*)s_starmapSrv);
        g_pRenderer->SetObjectAdditiveBlend(true);
    ...
    if (!s_starmapSrv and sb > 0.003f)
    {
        ...
        if (sd and sd->coord and sd->totalcoord > 0)
        {
            ...
            const float ssz =
                (g_fSkyDomeStarSize > 0.0f ? g_fSkyDomeStarSize : 0.0011f) * R;
            ...
            {
                if (c->flag)
                    continue; // behind the sun / moon
                if (c->z > -0.02f)
                    continue; // up = -z: skip at/below the horizon
                float bright = (float)c->color * (1.0f / 255.0f) * sb;
                if (bright < 0.05f)
                    continue; // cull the dimmest
                if (bright > 1.0f)
                    bright = 1.0f;
                ...
                float rl = (float)sqrt(rx * rx + ry * ry);
                if (rl < 1e-6f)
                {
                    rx = 1.0f;
                    ry = 0.0f;
                ...
                rx /= rl;
                ry /= rl;
                const float ux = dys * rz - dzs * ry, uy = dzs * rx - dxs * rz,
                            uz = dxs * ry - dys * rx;
                const float cx = dxs * R, cy = dys * R, cz = dzs * R;
                const float aa = (float)sqrt(
                    bright); // compensate the object shader's colour*alpha premultiply
                const unsigned long col = PackSkyColor(1.0f, 1.0f, 1.0f, aa);
                const float s2 =
                    ssz *
                    (0.7f + 0.6f * bright); // brighter stars a touch larger
                for (int k = 0; k < 4; ++k)
                {
                    const float ex = ((k & 1) ? 1.0f : -1.0f) * s2,
                                ey = ((k & 2) ? 1.0f : -1.0f) * s2;
                    SkyDomeVert& o = sv[q * 4 + k];
                    o.p[0] = cx + rx * ex + ux * ey;
                    o.p[1] = cy + ry * ex + uy * ey;
                    o.p[2] = cz + rz * ex + uz * ey;
                    o.n[0] = 0;
                    o.n[1] = 0;
                    o.n[2] = -1;
                    o.col = col;
                ...
                ip[0] = b0;
                ip[1] = (unsigned short)(b0 + 1);
                ip[2] = (unsigned short)(b0 + 2);
                ip[3] = (unsigned short)(b0 + 1);
                ip[4] = (unsigned short)(b0 + 3);
                ip[5] = (unsigned short)(b0 + 2);
                if (++q >= MAXST)
                    break;
            }
            if (q > 0)
            {
                g_pRenderer->BeginSkyPass(
                    true); // alpha blend over the gradient dome; drawn before sun/moon
                g_pRenderer->SetProj(
    ...
    if (TheTimeOfDay.ThereIsASun() and sunElev > 0.0f) // sun above the horizon
    {
        ...
        const float sunSz =
            (g_fSkyDomeSunSize > 0.0f ? g_fSkyDomeSunSize : 0.010f);
        float
            gRd = 1.0f,
            gGd = 1.0f,
            gBd =
                1.0f; // disc tint = atmospheric extinction (set in the glare block)
        {
            const float sh =
                (sunUp > 1.0f) ? 1.0f : sunUp; // sin(elevation), >0 here
            const float shDeg = (float)(asin(sh) * 180.0 / PI);
            float Xs =
                1.0f / (sh + 0.50572f * (float)pow(shDeg + 6.07995, -1.6364));
            if (Xs < 1.0f)
                Xs = 1.0f;
            if (Xs > 12.0f)
                Xs = 12.0f;
            const float gR = (float)pow(
                10.0,
                -0.4 * 0.09 *
                    (Xs - 1.0f)); // per-channel extinction: blue dies first
            const float gG = (float)pow(10.0, -0.4 * 0.14 * (Xs - 1.0f));
            const float gB = (float)pow(10.0, -0.4 * 0.22 * (Xs - 1.0f));
            // Artscout - 2026: hand the extinction to the DISC as a HUE shift only -- NORMALIZED so its brightest
            // channel stays 1. Passing it raw made the low sun a dark BROWN hole: at 5 deg the airmass gives
            // (0.47,0.31,0.16), while the glare beside it is ADDITIVE over an already-bright sky, so the disc
            // came out DARKER than the sky around it. The raw factor is right for the direct beam and wrong for
            // an LDR image: the real sun stays ~10^5x brighter than the sky, so it is always the brightest thing
            // on screen no matter how much air it crosses -- what the airmass actually changes is its COLOUR.
            // Normalized, 5 deg gives (1.00,0.66,0.34): a bright orange sunset disc, which is what one looks like.
            float gMax = gR;
            if (gG > gMax)
                gMax = gG;
            if (gB > gMax)
            ...
            if (glare)
                DrawSkyBillboard(&sunD, R, SUN_GLARE_HALF * R, gR, gG, gB,
                                 glare, 1.0f, 1.0f, /*additive*/ true);
        }
    ...
    {
        Tpoint moonD;
        TheTimeOfDay.CalculateSunMoonPos(&moonD, TRUE);
        ...
        if (moonGate)
        {
            ...
            static bool s_moonHalo =
                false; // baked canvas carries a glow ring (billboard must cover 2x the disc)
            if (!s_moonTried && g_pRenderer)
            {
                ...
                if (void* hi = (void*)g_pRenderer->LoadTextureFile(
                        "terrdata/misctex/moon.png"))
                {
                    s_moonSrv = hi;
                    s_moonHalo = false;
                }
                else if (void* hd = (void*)g_pRenderer->LoadTextureFile(
                             "terrdata/misctex/moon.dds"))
                {
                    s_moonSrv = hd;
                    s_moonHalo = false;
                }
                ...
                if (!s_moonSrv and dim > 0 and src and
                    pal) // only bake the gif when no high-res moon.png/.dds was loaded
                {
                    ...
                    const unsigned chroma =
                        (unsigned)(mt.chromaKey & 0xFFFFFFu);
                    const int HALO = 2;
                    const int cdim = dim * HALO;
                    const float cc = (cdim - 1) * 0.5f; // canvas centre
                    const float off =
                        (cdim - dim) *
                        0.5f; // where the source disc starts in the canvas
                    ...
                    for (int Y = 0; Y < cdim; ++Y)
                        for (int X = 0; X < cdim; ++X)
                        {
                            ...
                            const float sx = (float)X - off,
                                        sy = (float)Y - off;
                            const int x0 = (int)floorf(sx),
                                      y0 = (int)floorf(sy);
                            const float fx = sx - x0, fy = sy - y0;
                            float aAcc = 0.0f, rAcc = 0.0f, gAcc = 0.0f,
                                  bAcc = 0.0f;
                            for (int j = 0; j < 2; ++j)
                                for (int i = 0; i < 2; ++i)
                                {
                                    const int xi = x0 + i, yi = y0 + j;
                                    const float w = (i ? fx : 1.0f - fx) *
                                                    (j ? fy : 1.0f - fy);
                                    float ta = 0.0f, R = 0.0f, G = 0.0f,
                                          B = 0.0f;
                                    if (xi >= 0 and xi < dim and yi >= 0 and
                                        yi < dim)
                                    {
                                        const unsigned rgb =
                                            pal->paletteData[src[yi * dim +
                                                                 xi]] &
                                            0xFFFFFFu;
                                        R = (float)((rgb >> 16) & 0xFFu);
                                        G = (float)((rgb >> 8) & 0xFFu);
                                        B = (float)(rgb & 0xFFu);
                                        if (rgb != chroma)
                                        {
                                            ...
                                            ta =
                                                ...
                                                (3.0f -
                                                 2.0f *
                                                     s); // smoothstep -> fade the near-black limb
                                        }
                                    }
                                    aAcc += w * ta;
                                    rAcc += w * ta * R;
                            ...
                            const float mA =
                                aAcc; // anti-aliased moon coverage 0..1
                            float
                                mr = 205.0f,
                                mg = 214.0f,
                                mb = 235.0f; // where transparent -> glow colour
                            if (aAcc > 1e-4f)
                            {
                                mr = rAcc / aAcc;
                                mg = gAcc / aAcc;
                            ...
                            const unsigned R8 =
                                (unsigned)(gr + (mr - gr) * mA + 0.5f);
                            const unsigned G8 =
                                (unsigned)(gg + (mg - gg) * mA + 0.5f);
                            const unsigned B8 =
                                (unsigned)(gb + (mb - gb) * mA + 0.5f);
                            if (mA >= 0.5f)
                                ++opaque;
                            const unsigned A8 =
                                (unsigned)(255.0f * (A < 1.0f ? A : 1.0f) +
                                           0.5f);
                            rgba[p] = (A8 << 24) | (B8 << 16) | (G8 << 8) |
                                      R8; // R8G8B8A8 (R,G,B,A)
                        }
                    if (opaque > 0)
                    {
                        s_moonSrv = (void*)g_pRenderer->LoadTextureRGBA(
                            rgba, cdim, cdim);
            ...
            const float mSize =
                (g_fSkyDomeMoonSize > 0.0f ? g_fSkyDomeMoonSize : 0.030f);
            ...
            const float mA =
                0.20f +
                0.80f * mNight; // opacity: 0.20 (pale daytime) .. 1.0 (night)
            ...
            float X =
                1.0f / (mh + 0.50572f * (float)pow(hDeg + 6.07995, -1.6364));
            if (X < 1.0f)
                X = 1.0f;
            if (X > 12.0f)
                X = 12.0f;
            const float kR = 0.09f, kG = 0.14f,
                        kB = 0.22f; // mag/airmass: blue is scattered most
            ...
            const float dDeg = (float)(2.0 * mSize * 180.0 /
                                       PI); // angular DIAMETER of the disc
            float hTop = hDeg + dDeg * 0.5f, hBot = hDeg - dDeg * 0.5f;
            if (hBot < 0.0f)
                hBot = 0.0f;
#define BENNETT_DEG(H)                                                         \
    ((float)(1.0 / tan(((H) + 7.31 / ((H) + 4.4)) * PI / 180.0) / 60.0))
...
#undef BENNETT_DEG
            ...
            if (s_moonSrv and !s_moonHalo and mG > 0.02f and mglare)
                DrawSkyBillboard(&moonD, R, MOON_GLARE_HALF * R,
                                 tR * 0.80f * mG, tG * 0.84f * mG,
                                 tB * 0.92f * mG, mglare, 1.0f, flat,
                                 /*additive*/ true);
            DrawSkyBillboard(&moonD, R, sz * R, 0.98f * tR, 0.98f * tG,
                             1.0f * tB, s_moonSrv, mA, flat);
        }
    }
}
...
\***************************************************************************/
int RenderOTW::DrawCelestialBody(Tpoint* cntr, float dist, float alpha, float r,
                                 float g, float b)
{
    ThreeDVertex v0, v1, v2, v3;
    Tpoint eastSide;
...
void RenderOTW::ComputeHorizonEffect(HorizonRecord* pHorizon)
{
    ...
    else
    {
        ...
        else
        {
            ...
            pHorizon->hazescale =
                0.6f + 0.4f * (float)-deltapitch / (float)bandangle;
    ...
    if (1 /* not (sunEffectScreenSpace.clipFlag bitand CLIP_NEAR)*/)
    {
        ...
        Edge horizonLine;
        horizonLine.SetupWithVector(shiftX + pHorizon->vx,
                                    shiftY + pHorizon->vy, pHorizon->hx,
                                    pHorizon->hy);
...
void RenderOTW::SetTimeOfDayColor(void)
{
    ...
    if (TheTimeOfDay.GetNVGmode())
    {
        ...
        DWORD ground_haze =
            (FloatToInt32(haze_ground_color.g * 255.9f) << 8) + 0xff000000;
    ...
    else
    {
        ...
        lightPhi =
            (float)atan2(-lightVector.z, sqrt(lightVector.x * lightVector.x +
        ...
        DWORD ground_haze = (FloatToInt32(haze_ground_color.r * 255.9f)) +
                            ...
                            (FloatToInt32(haze_ground_color.b * 255.9f) << 16) +
                            0xff000000;
...
void RenderOTW::AdjustSkyColor(void)
{
    ...
    // residual off-axis sky-band gap blends with the sky rather than a fixed colour.
    {
        ...
        g_vrClearColor[0] = sky_color.r;
        g_vrClearColor[1] = sky_color.g;
        g_vrClearColor[2] = sky_color.b;
    }
}
