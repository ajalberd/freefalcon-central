...
**Status: not started.** Reconnaissance only — what the renderer does today and which of these
four asks are independent. Append findings here rather than growing `WIP-NOTES.md`.

## The goal

Four things, and they are not one project:

1. Redo the lighting system.
2. Real shadows in the cockpit.
3. Fix the missile shading issue.
4. Integrate Gran Turismo 7 tone mapping.
...
## The missile shading issue = the fin flicker

**Confirmed by the user, 2026-09-18.** This is the long-standing **missile fin flicker**
already listed in `WIP-NOTES.md` under Known open — not a separate defect. Still unexplained.

Closed avenue, do not re-chase: per-surface `dwzBias` was restored and did **not** fix it.
(That field, for the object pass, is at `irenderer.h:122`.)

Next move is a **RenderDoc capture of two consecutive frames**, not more code reading. Three
confident code-reads have already lost to one measurement on this codebase — see the
`freefalcon-instrument-first` note. What to compare between the two frames: which draw emits
the fin surfaces, whether it is the same draw both frames, the surface's depth-bias and
render state, and whether the vertices move. "Flicker" could be geometry appearing and
disappearing, z-fighting, a state that alternates, or a surface that alternates between two
draw batches; those look identical in motion and completely different in a capture.

## Suggested order

These are listed in the order they were asked, which is not the order to do them.

1. **Missile fin flicker** — smallest, independent of the rest, and an existing annoyance.
   Start with a capture, not with code.
...
3. **Lighting redo + HDR + GT7 tone mapping** — one project, not two. The tone curve is the
   last step of it, not a feature that can land first.
